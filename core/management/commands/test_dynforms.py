...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if superuser:
            self.stdout.write(
                self.style.SUCCESS(f"✅ Superuser found: {superuser.username}")
            )
            ...
            try:
                ...
                if admin_role in superuser.roles.all():
                    ...
                    self.stdout.write(
                        self.style.WARNING("⚠️  Adding admin role to superuser...")
                    )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✅ django-dynforms installed: v{version}")
            )
        except ImportError:
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS("✅ Dynforms FormType model accessible")
            )
        ...
        if os.path.exists(staticfiles_dir):
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✅ Static files: {len(files)} files")
            )
            ...
            if missing:
                self.stdout.write(
                    self.style.WARNING(f"   ⚠️  Missing: {', '.join(missing)}")
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS("   ✅ All required files present")
                )
        else:
        ...
        if os.path.exists(templates_dir):
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✅ Custom templates: {', '.join(template_files)}")
            )
        else:
