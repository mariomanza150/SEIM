...
1. Copy `tenant_config.json.example` to `tenant_config.json` (gitignored) **or** copy `branding\uadec\` to `branding\<your-slug>\`.
2. Edit the JSON (name, short name, email, colors, website).
3. Put logos in `branding\<your-slug>\logos\` (PNG/SVG). After `collectstatic` they are served as `/static/<your-slug>/logos/...`.
4. Optionally also copy `branding\institution.json.example` to `branding\institution.json`.
...
Existing UAdeC deployments keep working if you change nothing (`tenant_config.json` is optional).

## Precedence

1. Environment variables (`INSTITUTION_*`, `BRAND_*`, `INSTITUTION_SLUG`, `TENANT_CONFIG_FILE`)
2. `tenant_config.json` (repo root; copy `tenant_config.json.example`) or `TENANT_CONFIG_FILE`
3. `INSTITUTION_CONFIG_FILE` (default `branding\institution.json`)
4. Packaged defaults in `branding\<slug>\config.json` (slug from env / overlay, default `uadec`)
5. Built-in Python defaults in `core\branding.py` (UAdeC fallback)
...
| `TENANT_CONFIG_FILE` | Path to root tenant overlay (default `tenant_config.json`) |
| `INSTITUTION_CONFIG_FILE` | Path to a JSON overlay (`branding\institution.json`) |
...
`download_uadec_assets.py` (repo root and `scripts\`) is a compatibility wrapper. It reads `tenant_config.json` plus:

- `TENANT_CONFIG_FILE` — overlay JSON (default `tenant_config.json`)
- `INSTITUTION_SLUG` — branding pack folder (default `uadec`)
- `INSTITUTION_WEBSITE` — homepage to scrape (from tenant/branding config; UAdeC fallback)
