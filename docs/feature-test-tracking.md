...
| `vue-portal` | Vue student/coordinator portal (shell, routing, a11y) | Done | Partial | Partial | 2026-04-09 | **Unit (green, Vitest):** `App.spec.js`, `Dashboard.spec.js`, `Login.spec.js`, `NotFound.spec.js`, `utils/a11y.spec.js`, `utils/documentTitle.spec.js` (i18n browser tab titles), `components/ToastContainer.spec.js`, `components/NotificationDropdown.spec.js`, `i18n/index.spec.js`, `utils/dashboardNextSteps.spec.js`, `services/uiPreferences.spec.js`, `services/websocket.spec.js` (`npm run test:run -- --run` with those paths). Feature views also have specs under other clusters. **Smoke/Browser:** host Playwright `test_vue_ui.py` (`-m vue`); same Docker browser-lib constraint as `auth-api`. |
| `cms-public` | Wagtail CMS public site | Done | Partial | Partial | 2026-04-09 | **Unit (green):** `tests/unit/cms/` — 9 tests with **`DJANGO_SETTINGS_MODULE=seim.settings.test_cms`** (Wagtail + `cms` enabled; `seim/settings/test_cms.py` extends `test` but keeps full `INSTALLED_APPS` minus `cacheops`). Run: `docker compose exec web sh -c "unset DATABASE_URL && export DJANGO_SETTINGS_MODULE=seim.settings.test_cms && pytest tests/unit/cms/ -o addopts='--strict-markers --tb=short'"`. Default `seim.settings.test` **cannot** collect these (Wagtail stripped). **Smoke:** `tests/e2e_playwright/test_smoke.py::test_homepage_loads` (and mobile variant) when `BASE_URL` is the site root — **host** Playwright / same Docker browser constraint as `auth-api`. **Browser:** broader marketing/CMS journeys often manual or video demos; not re-run here. |
| `url-routing` | Global URL map (root vs `/seim/` SPA, admin redirects, API) | Done | — | — | 2026-04-09 | **Unit (green):** `tests/test_urls.py` — 12 tests with `DJANGO_SETTINGS_MODULE=seim.settings.test` (`pytest tests/test_urls.py -o addopts='--strict-markers --tb=short'`). `seim/settings/test.py` prepends `tests/fixtures/templates/` so `TemplateView` `index.html` resolves without `frontend-vue/dist`. Assertions branch on `django.apps.apps.is_installed("wagtail")` for `/` and `/cms/`. **Smoke/Browser:** not mapped. |
...
| `settings-ui` | User settings (appearance, notifications, i18n) | Done | Partial | Partial | 2026-04-09 | **Unit (green):** `Settings.spec.js` (5 tests): load/save, UI language `LOCALE_STORAGE_KEY`, staff notification-routing link; form + sidebar strings via `settings.*` / `route.names` / `applicationFormPage.cancel`. **Smoke:** host Playwright + `BASE_URL`. **Browser:** `test_student_workflows.py::test_view_settings`; POM `settings_page.py`. |
...
*Last updated: 2026-04-09 — Settings page **full i18n** (`settings.*`). Prior: `data-management` on `testpaths`; `url-routing` row.*
