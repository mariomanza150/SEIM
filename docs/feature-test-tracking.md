...
| `notifications` | Notifications center, real-time, digests, routing reference | Partial | — | — | 2026-04-09 | Routing ref (schema v9, `digest.recipient_summary` + transactional `recipient_summary`): `test_routing_reference.py`, `test_notification_routing_reference_api.py`, `test_notifications_tasks.py`; Vue `NotificationRouting.spec.js`, `Settings.spec.js`. WebSocket / inbox E2E still open. |
...
*Last updated: 2026-04-09 — Notification routing **schema v9** (`digest.recipient_summary`). Prior: `readiness-compare` verified.*
