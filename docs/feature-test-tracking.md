...
| `notifications` | Notifications center, real-time, digests, routing reference | Done | Partial | Partial | 2026-04-09 | **Unit (green):** 148 tests — `tests/unit/notifications/` (incl. `test_notification_center.py`, `test_websocket_consumer.py`, digest/tasks/services) + `tests/integration/api/test_notification_routing_reference_api.py` + `tests/integration/test_websocket_notifications.py`. Run in Docker: `sh -c "unset DATABASE_URL && export DJANGO_SETTINGS_MODULE=seim.settings.test && pytest … -o addopts='--strict-markers --tb=short'"` so SQLite in-memory is used (`seim.settings.test`); `pytest-asyncio` is installed in `Dockerfile` web image; root `pytest.ini` sets `asyncio_mode = auto` + `asyncio` marker. Vue: `Notifications.spec.js`, `NotificationRouting.spec.js` (6 Vitest). **Smoke/Browser:** host Playwright not re-run; WebSocket/inbox journeys same constraint as `auth-api`. |
...
*Last updated: 2026-04-09 — `notifications` cluster unit layer green (148 pytest + 6 Vitest); `pytest-asyncio` in web Dockerfile; async consumer tests use `sync_to_async` for ORM.*
