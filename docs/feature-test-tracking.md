...
*Last updated: 2026-04-09 — `programs-applications` unit verified (Docker + Vue specs above); `test_applications_api` fixes for `?status=draft` and admin dynamic-form create. Notification routing / `settings-ui` notes from earlier edits preserved in matrix rows.*
