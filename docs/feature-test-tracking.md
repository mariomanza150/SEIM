...
| `notifications` | Notifications center, real-time, digests, routing reference | Partial | — | — | 2026-04-09 | Routing ref (schema v8, `transactional_routes.recipient_summary` + `primary_recipients`): `test_routing_reference.py`, `test_notification_routing_reference_api.py`, `test_notifications_tasks.py`; Vue `NotificationRouting.spec.js`, `Settings.spec.js`. WebSocket / inbox E2E still open. |
...
*Last updated: 2026-04-09 — Notification routing **schema v8** (`recipient_summary` on transactional routes). Prior: `analytics` unit slice.*
