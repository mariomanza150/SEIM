...
| `cms-public` | Wagtail CMS public site | Done | Partial | Partial | 2026-04-09 | **Unit (green):** `tests/unit/cms/` — 9 tests with **`DJANGO_SETTINGS_MODULE=seim.settings.test_cms`** (Wagtail + `cms` enabled; `seim/settings/test_cms.py` extends `test` but keeps full `INSTALLED_APPS` minus `cacheops`). Run: `docker compose exec web sh -c "unset DATABASE_URL && export DJANGO_SETTINGS_MODULE=seim.settings.test_cms && pytest tests/unit/cms/ -o addopts='--strict-markers --tb=short'"`. Default `seim.settings.test` **cannot** collect these (Wagtail stripped). **`tests/test_urls.py`** Wagtail/root assertions are outdated vs current routing (legacy `TestCase`); not part of this cluster’s green bundle. **Smoke:** `tests/e2e_playwright/test_smoke.py::test_homepage_loads` (and mobile variant) when `BASE_URL` is the site root — **host** Playwright / same Docker browser constraint as `auth-api`. **Browser:** broader marketing/CMS journeys often manual or video demos; not re-run here. |
...
*Last updated: 2026-04-09 — Vue **i18n browser tab titles** (`documentTitle.spec.js`). Prior: `cms-public` test_cms; notification routing schema v12.*
