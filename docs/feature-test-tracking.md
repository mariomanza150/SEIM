...
| `notifications` | Notifications center, real-time, digests, routing reference | Partial | — | — | 2026-04-09 | Routing ref (schema v5, `reference_api_access`): `test_routing_reference.py`, `test_notification_routing_reference_api.py`, `test_notifications_tasks.py`; Vue `NotificationRouting.spec.js`, `Settings.spec.js`. WebSocket / inbox E2E still open. |
...
*Last updated: 2026-04-09 — `dynamic-forms` unit verified (152 pytest + Vue specs); `FormType.created_by` `blank=True` + `0005` migration; serializer test matches service-layer required-field rules. Notifications schema v5 note preserved in matrix row.*
