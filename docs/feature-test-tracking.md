...
| `notifications` | Notifications center, real-time, digests, routing reference | Partial | — | — | 2026-04-09 | Routing ref (schema v7, `transactional_routes` + `primary_recipients`): `test_routing_reference.py`, `test_notification_routing_reference_api.py`, `test_notifications_tasks.py`; Vue `NotificationRouting.spec.js`, `Settings.spec.js`. WebSocket / inbox E2E still open. |
...
*Last updated: 2026-04-09 — `analytics` unit verified (151 pytest + test-setting/template fixes). Prior: notification routing schema v7 note in matrix row.*
