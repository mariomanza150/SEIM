...
### Demo seed
`manage.py seed_demo_readiness` seeds a walkthrough dataset: demo users (including partner + nominated student), programs with host destination trees, exchange agreements, applications across statuses (including waitlist and nominated), nomination cycles/allocations, documents, comments, timeline, notifications, eligibility rulesets, dynamic forms, workflows, scholarships, partner portal contacts, reminders, saved searches, grade translations, TOEFL practice attempt history, data-management sample logs, plus best-effort `restore_cms` / `seed_spa_help`. Use `manage.py create_initial_data` first if needed (roles, statuses including `nominated`, document/notification types, allowed email domains, mobility schemes, profile catalogs) plus a manually created superuser. Demo credentials: `admin@test.com` / `admin123`, `coordinator@test.com` / `coordinator123`, `student@test.com` / `student123`, `partner@test.com` / `partner123`. Partner acknowledge demo: `student.nominated@test.com` / `student123`. TOEFL Launch needs the sidecar (`docker-compose.toefl.yml` + `TOEFL_*` in `.env`; see `env.example`).
