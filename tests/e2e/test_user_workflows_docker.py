...
class TestUserWorkflowsDocker:
    def test_registration_workflow(self, driver):
        ...
        assert (
            driver.find_element(By.ID, "username").get_attribute("value") == "newuser"
        )
        assert (
            driver.find_element(By.ID, "email").get_attribute("value")
            == "newuser@example.com"
        )
        ...
        assert (
            current_url == f"{BASE_URL}/register/"
            or current_url == f"{BASE_URL}/login/"
        )
    ...
    def test_login_workflow(self, driver):
        ...
        assert (
            driver.find_element(By.ID, "username").get_attribute("value") == "testuser"
        )
        assert (
            driver.find_element(By.ID, "password").get_attribute("value")
            == "password123"
        )
    ...
    def test_application_submission_workflow(self, driver):
        ...
        driver.find_element(By.ID, "personal_statement").send_keys(
            "Test personal statement"
        )
        driver.find_element(By.ID, "academic_background").send_keys(
            "Test academic background"
        )
        driver.find_element(By.ID, "language_proficiency").send_keys(
        ...
        assert (
            driver.find_element(By.ID, "personal_statement").get_attribute("value")
            == "Test personal statement"
        )
        assert (
            driver.find_element(By.ID, "academic_background").get_attribute("value")
            == "Test academic background"
        )
        assert (
            driver.find_element(By.ID, "language_proficiency").get_attribute("value")
            == "Test language proficiency"
        )
        assert (
            driver.find_element(By.ID, "financial_plan").get_attribute("value")
            == "Test financial plan"
        )
