...
class TestAssignUserRolesCommand(TestCase):
    ...
    def test_assign_user_roles_command_with_user_id(self):
        ...
        out = StringIO()
        call_command(
            "assign_user_roles", username=self.user.username, role="student", stdout=out
        )
    ...
    def test_assign_user_roles_command_invalid_user(self):
        ...
        out = StringIO()
        call_command(
            "assign_user_roles", username="nonexistentuser", role="student", stdout=out
        )
...
class TestCreateMissingProfilesCommand(TestCase):
    ...
    def test_create_missing_profiles_command_dry_run(self):
        ...
        output = out.getvalue()
        self.assertTrue(
            "DRY RUN" in output or "All users already have profiles!" in output
        )
