...
class TestSendNotificationByIdTask:
    ...
    def test_send_notification_by_id_multiple_notifications(self, user, another_user):
        ...
        notif1 = Notification.objects.create(
            recipient=user, title="Notification 1", message="Message 1", category="info"
        )
    ...
    @patch("notifications.tasks.send_mail")
    def test_send_notification_by_id_fail_silently_false(
        self, mock_send_mail, notification
    ):
        """Test that fail_silently is False."""
        mock_send_mail.side_effect = Exception("SMTP error")
    ...
    def test_send_notification_by_id_with_empty_title(self, user):
        ...
        notif = Notification.objects.create(
            recipient=user, title="", message="Message with no title", category="info"
        )
    ...
    def test_send_deadline_reminders_notification_type(self, user):
        ...
        with patch(
            ...
            mock_send.return_value = Notification.objects.create(
                recipient=user, title="Test", message="Test", category="warning"
            )
        ...
        with patch(
            ...
            mock_send.side_effect = [
                ...
                Notification.objects.create(
                    recipient=user, title="Success", message="Success", category="info"
                ),
            ]
