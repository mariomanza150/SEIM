...
class TestGenerateReportTask:
    def test_generate_report_task_success(self):
        ...
        Application.objects.create(
            program=program, student=user, status=submitted_status
        )
        Application.objects.create(
            program=program, student=user, status=approved_status
        )
        Application.objects.create(
            program=program, student=user, status=rejected_status
        )
    ...
    def test_generate_report_task_with_existing_metrics(self):
        ...
        Metric.objects.create(name="applications_submitted", value=5.0, report=report)
        # Execute the task
        tasks.generate_report(report.id)
        ...
        values = list(
            metrics.filter(name="applications_submitted").values_list(
                "value", flat=True
            )
        )
