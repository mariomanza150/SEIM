...
class TestProgramFilter:
    ...
    def test_filter_by_name(self, programs):
        ...
        filterset = ProgramFilter(
            data={"name": "Erasmus"}, queryset=Program.objects.all()
        )
    ...
    def test_filter_by_active_status(self, programs):
        ...
        filterset = ProgramFilter(
            data={"is_active": True}, queryset=Program.objects.all()
        )
    ...
    def test_filter_by_gpa(self, programs):
        ...
        filterset = ProgramFilter(
            data={"min_gpa_max": 3.0}, queryset=Program.objects.all()
        )
    ...
    def test_filter_by_language(self, programs):
        ...
        filterset = ProgramFilter(
            data={"required_language": "English"}, queryset=Program.objects.all()
        )
    ...
    def test_search_full_text(self, programs):
        ...
        filterset = ProgramFilter(
            data={"search": "exchange"}, queryset=Program.objects.all()
        )
    ...
    def test_ordering(self, programs):
        ...
        filterset = ProgramFilter(
            data={"ordering": "start_date"}, queryset=Program.objects.all()
        )
    ...
    def test_filter_accepting_applications(self, programs):
        ...
        for p in fs.qs:
            open_ok = (
                p.application_open_date is None or p.application_open_date <= today
            )
            deadline_ok = (
                p.application_deadline is None or p.application_deadline >= today
            )
...
class TestApplicationFilter:
    ...
    def test_filter_by_student_name(self, applications):
        ...
        filterset = ApplicationFilter(
            data={"student_name": student.username}, queryset=Application.objects.all()
        )

        assert filterset.is_valid()
        results = list(filterset.qs)
        assert all(
            student.username.lower() in a.student.username.lower()
            or student.username.lower() in (a.student.first_name or "").lower()
            or student.username.lower() in (a.student.last_name or "").lower()
            for a in results
        )
    ...
    def test_filter_by_student_email(self, applications):
        ...
        filterset = ApplicationFilter(
            data={"student_email": student.email}, queryset=Application.objects.all()
        )
    ...
    def test_filter_by_program_name(self, applications):
        ...
        filterset = ApplicationFilter(
            data={"program_name": program.name}, queryset=Application.objects.all()
        )
    ...
    def test_filter_by_status(self, applications):
        ...
        filterset = ApplicationFilter(
            data={"status_name": status.name}, queryset=Application.objects.all()
        )
    ...
    def test_filter_by_submitted_date(self, applications):
        ...
        results = list(filterset.qs)
        assert all(
            a.submitted_at is None or a.submitted_at >= yesterday for a in results
        )

    def test_filter_active_applications(self, applications):
        """Test filtering active (not withdrawn) applications."""
        filterset = ApplicationFilter(
            data={"active": True}, queryset=Application.objects.all()
        )
    ...
    def test_search_multiple_fields(self, applications):
        ...
        filterset = ApplicationFilter(
            data={"search": student.username[:5]}, queryset=Application.objects.all()
        )
...
class TestApplicationReviewQueueFilters:
    ...
    def test_pending_review_true(self, db_with_roles, student_user):
        ...
        Application.objects.create(
            student=student_user, program=program, status=st_draft
        )

        fs = ApplicationFilter(
            data={"pending_review": True}, queryset=Application.objects.all()
        )
        assert fs.is_valid()
        assert fs.qs.count() == 1
        assert fs.qs.first().status.name == "submitted"

    def test_needs_document_resubmit(
        self, db_with_roles, student_user, coordinator_user
    ):
        program = Program.objects.create(
            name="Doc Program",
            description="Test",
        ...
        fs = ApplicationFilter(
            data={"needs_document_resubmit": True}, queryset=Application.objects.all()
        )
        assert fs.is_valid()
...
def applications(db, user_student, programs):
    ...
    submitted_status = ApplicationStatus.objects.get_or_create(
        name="submitted", defaults={"order": 1}
    )[0]
