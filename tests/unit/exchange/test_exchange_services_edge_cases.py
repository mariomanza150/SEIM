...
from exchange.models import Application, ApplicationStatus, Comment, Program, TimelineEvent
...
class TestApplicationServiceEdgeCases(TestCase):
    ...
    def test_program_with_negative_capacity(self):
        """Test program with negative capacity (should be prevented)."""
        # Negative capacity doesn't make sense, but test the model handles it
        program = Program.objects.create(
            ...
            end_date=timezone.now().date() + timezone.timedelta(days=365),
            is_active=True,
            max_participants=0,  # Zero is valid (unlimited)
        )
        
        self.assertEqual(program.max_participants, 0)
...
class TestErrorHandlingEdgeCases(TestCase):
    ...
    def test_comment_on_nonexistent_application(self):
        """Test adding comment to deleted application."""
        program = Program.objects.create(
            name="Test Program",
            description="Test",
            start_date=timezone.now().date(),
        ...
        # Try to create comment (should fail with IntegrityError or DoesNotExist)
        with self.assertRaises(Exception):
            Comment.objects.create(
                application_id=app_id,  # Deleted application
                author=self.student,
                text="Comment on deleted application",
            )
...
class TestBoundaryConditions(TestCase):
    ...
    def test_max_participants_boundary(self):
        """Test maximum participants boundary values."""
        # Zero (unlimited)
        program1 = Program.objects.create(
        ...
        self.assertEqual(program1.max_participants, 0)
        ...
        program2 = Program.objects.create(
            ...
            end_date=timezone.now().date() + timezone.timedelta(days=365),
            is_active=True,
            max_participants=999999,  # Very large
        )
        self.assertEqual(program2.max_participants, 999999)
