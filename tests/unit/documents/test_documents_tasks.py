...
class TestScanDocumentVirusTask:
    ...
    def test_scan_document_virus_clean_file(self, document, validator_user):
        ...
        with patch("documents.virus_scanner.scan_file_for_viruses") as mock_scan:
            ...
            validation = DocumentValidation.objects.filter(
                document=document, validator_id=validator_id
            ).first()
    ...
    def test_scan_document_virus_updates_existing_validation(
        self, document, validator_user
    ):
    ...
    def test_scan_document_virus_multiple_concurrent_scans(
        self, document, validator_user, db
    ):
        ...
        with patch("documents.virus_scanner.scan_file_for_viruses") as mock_scan:
            ...
            assert DocumentValidation.objects.filter(
                document=document, validator_id=validator_id_1
            ).exists()

            assert DocumentValidation.objects.filter(
                document=document, validator_id=validator_id_2
            ).exists()
    ...
    def test_scan_document_virus_preserves_document_metadata(
        self, document, validator_user
    ):
    ...
    def test_scan_document_virus_secondary_error_handling(
        self, document, validator_user
    ):
        ...
        with patch("documents.virus_scanner.scan_file_for_viruses") as mock_scan:
            ...
            with patch(
                "documents.models.DocumentValidation.objects.create"
            ) as mock_create:
    ...
    def test_scan_document_virus_idempotency(self, document, validator_user):
        ...
        with patch("documents.virus_scanner.scan_file_for_viruses") as mock_scan:
            ...
            assert DocumentValidation.objects.filter(document=document).count() == 3
