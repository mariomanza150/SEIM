...
class TestAPIValidationEdgeCases(TestCase):
    ...
    def test_filter_with_invalid_values(self):
        ...
        response = self.client.get(
            reverse("api:program-list") + "?start_date=not-a-date"
        )
        self.assertIn(
...
class TestAPIErrorHandling(TestCase):
    ...
    def test_get_nonexistent_resource(self):
        ...
        response = self.client.get(reverse("api:application-detail", args=[fake_uuid]))
    ...
    def test_delete_already_deleted_resource(self):
        ...
        response = self.client.delete(reverse("api:application-detail", args=[app_id]))
    ...
    def test_partial_update_with_readonly_fields(self):
        ...
        self.client.patch(
            reverse("api:application-detail", args=[application.id]), data
        )
...
class TestAPIRateLimiting(TestCase):
    ...
    def test_rapid_fire_requests(self):
        ...
        sum(1 for code in responses if code == 429)
...
class TestApplicationStatusEdgeCases(TestCase):
    ...
    def setUp(self):
        ...
        self.submitted, _ = ApplicationStatus.objects.get_or_create(
            name="submitted", defaults={"order": 2}
        )
        self.under_review, _ = ApplicationStatus.objects.get_or_create(
            name="under_review", defaults={"order": 3}
        )
        self.approved, _ = ApplicationStatus.objects.get_or_create(
            name="approved", defaults={"order": 4}
        )
        self.rejected, _ = ApplicationStatus.objects.get_or_create(
