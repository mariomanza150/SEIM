...
class TestStudentApplicationWorkflow(TestCase):
    ...
    def setUp(self):
        ...
        self.approved_status, _ = ApplicationStatus.objects.get_or_create(
            name="approved", defaults={"order": 3}
        )
    ...
    def test_application_workflow_with_documents(self):
        ...
        uploaded_file = SimpleUploadedFile(
            "transcript.pdf", file_content, content_type="application/pdf"
        )
...
class TestCoordinatorWorkflow(TestCase):
    ...
    def test_coordinator_approves_application(self):
        ...
        ApplicationService.transition_status(
            self.application, self.coordinator, "approved"
        )
    ...
    def test_student_cannot_approve_application(self):
        ...
        with self.assertRaises(ValueError) as context:
            ApplicationService.transition_status(
                self.application, self.student, "approved"
            )
...
class TestGradeTranslationWorkflow(TestCase):
    ...
    def test_grade_translation_workflow(self):
        ...
        german_grade = GradeValue.objects.get(
            grade_scale=self.german_scale, numeric_value=1.0
        )

        # Create translation first (simulate existing translation)
        us_a = GradeValue.objects.get(grade_scale=self.us_scale, numeric_value=4.0)
    ...
    def test_gpa_conversion_workflow(self):
        ...
        # Get US grade B (3.0)
        us_b = GradeValue.objects.get(grade_scale=self.us_scale, numeric_value=3.0)
        ...
        german_2 = GradeValue.objects.get(
            grade_scale=self.german_scale, numeric_value=2.0
        )
...
class TestNotificationWorkflow(TestCase):
    ...
    def setUp(self):
        ...
        self.email_type, _ = NotificationType.objects.get_or_create(name="email")

        self.sms_type, _ = NotificationType.objects.get_or_create(name="sms")
    ...
    def test_notification_preference_workflow(self):
        ...
        NotificationService.set_preference(
            user=self.student, type_name="email", enabled=False
        )

        # Check preference
        is_enabled = NotificationService.is_enabled(self.student, "email")
...
class TestDynamicFormWorkflow(TestCase):
    ...
    def setUp(self):
        ...
        self.student = User.objects.create_user(
            username="formstudent", email="form@university.edu", password="TestPass123!"
        )
        self.student.roles.add(self.student_role)
    ...
    def test_dynamic_form_update_workflow(self):
        ...
        # Initial submission
        form_data1 = {"df_motivation": "Initial motivation"}
