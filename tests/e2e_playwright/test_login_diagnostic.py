...
def test_inspect_login_form_submission(page: Page, base_url: str):
    ...
    for i, form in enumerate(forms):
        ...
        print(
            f"  Form {i + 1}: action={form_action}, method={form_method}, id={form_id}"
        )
    ...
    sign_in_button = page.locator(
        'button:has-text("Sign In"), input[type="submit"]:has-text("Sign In")'
    ).first
    ...
    if sign_in_button.count() > 0:
        ...
        error_messages = page.locator(
            '.error, .alert-danger, .alert-error, [role="alert"], .message-error'
        )
