...
from tests.e2e_playwright.utils.auth_helpers import VueAppNotAvailable, login_as_admin
...
class TestAdminWorkflows:
    ...
    def test_admin_dashboard_access(self, page: Page, base_url: str):
        ...
        assert (
            ...
            or "dashboard" in title.lower()
            or "seim" in title.lower()
        ), f"Expected admin dashboard, got: {title}"

        # Check for admin-specific content
        (
            page.locator("text=/admin|system|management|analytics/i").count() > 0
            or page.locator(".admin-dashboard, .dashboard, .system").count() > 0
        )
    ...
    def test_access_all_features(self, page: Page, base_url: str):
        ...
        assert len(accessible_pages) > 0, (
            "Admin should be able to access at least some pages"
        )
        print(
            f"✅ Admin can access {len(accessible_pages)} pages: {', '.join(accessible_pages)}"
        )
    ...
    def test_program_management(self, page: Page, base_url: str):
        ...
        assert "login" not in page.url.lower(), (
            "Admin should be able to access program creation"
        )

        # Check for form or program management interface
        (
            page.locator("form").count() > 0
            or page.locator("text=/program|create|manage/i").count() > 0
        )

        print("✅ Admin can access program management")
        page.screenshot(
            path="tests/e2e_playwright/screenshots/admin_program_management.png"
        )
    ...
    def test_user_management(self, page: Page, base_url: str):
        ...
        (
            page.locator("text=/user|manage|role|permission/i").count() > 0
            or page.locator(".user-management, table, .list-group").count() > 0
        )

        print("✅ Admin can access user management")
        page.screenshot(
            path="tests/e2e_playwright/screenshots/admin_user_management.png"
        )
    ...
    def test_analytics_access(self, page: Page, base_url: str):
        ...
        (
            page.locator("text=/analytics|statistics|report|metric/i").count() > 0
            or page.locator(".analytics, .chart, .metric").count() > 0
        )
