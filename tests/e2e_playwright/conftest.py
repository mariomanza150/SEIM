...
try:
    ...
    pytest.skip(
        ...
        'Install test deps (pip install -e ".[test]") to run these tests.',
    ...
    if "video_demo" in video_demo_markers and videos_dir:
        ...
        if video_files:
            ...
            test_name_clean = (
                ...
                .replace("TestStudentVideoDemos_", "")
                .replace("TestCoordinatorVideoDemos_", "")
                .replace("TestAdminVideoDemos_", "")
                .replace("TestCrossRoleVideoDemos_", "")
            )
...
def login_as_student(page: Page, base_url: str) -> Generator[Page, None, None]:
    ...
    from tests.e2e_playwright.utils.auth_helpers import VueAppNotAvailable, login
...
@pytest.fixture
def authenticated_student_context(
...
@pytest.fixture
def authenticated_coordinator_context(
    browser: Browser, base_url: str
) -> Generator[BrowserContext, None, None]:
...
def screenshot_on_failure(request: pytest.FixtureRequest, page: Page):
    ...
    if request.node.rep_call.failed:
        ...
        page.screenshot(
            path=str(screenshot_path), full_page=SCREENSHOT_CONFIG["full_page"]
        )
...
def assert_visual_match(page: Page):
    ...
    def _assert_visual_match(name: str, threshold: float = None):
        ...
        return compare_screenshot(
            current_screenshot, baseline_path.read_bytes(), name, threshold
        )
