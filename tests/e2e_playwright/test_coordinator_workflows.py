...
class TestCoordinatorWorkflows:
    ...
    def test_coordinator_dashboard_access(self, page: Page, base_url: str):
        ...
        assert "login" not in page.url.lower(), (
            "Should be on coordinator dashboard, not login page"
        )

        # Check for coordinator dashboard elements
        title = page.title()
        assert (
            "coordinator" in title.lower()
        ...
        (
            page.locator("text=/coordinator|review|application|pending/i").count() > 0
            or page.locator(".coordinator-dashboard, .dashboard, .review").count() > 0
        )

        print("✅ Coordinator dashboard accessible")
        page.screenshot(
            path="tests/e2e_playwright/screenshots/coordinator_dashboard.png"
        )
    ...
    def test_view_all_applications(self, page: Page, base_url: str):
        ...
        assert "login" not in page.url.lower(), (
            "Should be able to access applications page"
        )

        # Coordinators should see all applications, not just their own
        # Check for applications list or table
        (
            page.locator("text=/application|all applications|review/i").count() > 0
            or page.locator(".application, .card, table, .list-group").count() > 0
        )

        print("✅ Coordinator can view applications page")
        page.screenshot(
            path="tests/e2e_playwright/screenshots/coordinator_applications.png"
        )
    ...
    def test_access_student_dashboard_features(self, page: Page, base_url: str):
        ...
        page.screenshot(
            path="tests/e2e_playwright/screenshots/coordinator_student_features.png"
        )
    ...
    def test_user_management_access(self, page: Page, base_url: str):
        ...
        page.screenshot(
            path="tests/e2e_playwright/screenshots/coordinator_user_management.png"
        )
    ...
    def test_analytics_access(self, page: Page, base_url: str):
        ...
        page.screenshot(
            path="tests/e2e_playwright/screenshots/coordinator_analytics.png"
        )
