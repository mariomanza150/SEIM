...
class TestAuthenticationSimple:
    ...
    def test_login_with_valid_credentials(self, page: Page, base_url: str):
        ...
        assert "login" not in current_url.lower(), (
            f"Should be on dashboard, but on: {current_url}"
        )
    ...
    def test_login_with_invalid_credentials(self, page: Page, base_url: str):
        ...
        email_field = page.locator(
            'input#email, input[type="email"], [data-testid="login-email"]'
        ).first
        try:
        ...
        assert "login" in current_url.lower(), (
            f"Expected to stay on login page, but redirected to: {current_url}"
        )
        ...
        has_error = (
            page.locator("text=/error|invalid|incorrect|wrong|credentials/i").count()
            > 0
            or page.locator(
        ...
        if has_error:
            ...
            print(
                "ℹ️  No explicit error message found, but correctly stayed on login page"
            )
    ...
    def test_login_with_invalid_credentials_api(self, page: Page, base_url: str):
        ...
        if response.status == 404:
            pytest.skip(
                "API not available at base_url. Run with BASE_URL=http://localhost:5173"
            )
        ...
        for dashboard_url in dashboard_urls:
            try:
                ...
                if "login" not in current_url.lower():
                    ...
                    page.screenshot(
                        path="tests/e2e_playwright/screenshots/dashboard_after_login.png"
                    )
                    break
            except:
                continue
