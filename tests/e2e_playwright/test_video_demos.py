...
class TestStudentVideoDemos:
    ...
    def test_demo_1_new_student_registration_first_application(
        ...
        registration_link = page.locator(
        ...
        if len(form_inputs) > 0:
            ...
            for _i, field in enumerate(form_inputs[:3]):
                try:
...
class TestCoordinatorVideoDemos:
    ...
    def test_demo_5_coordinator_request_resubmission(self, page: Page, base_url: str):
        ...
        resubmit_button = page.locator(
            'button:has-text("Resubmit"), a:has-text("Request Resubmission")'
        ).first
    ...
    def test_demo_6_coordinator_approve_application(self, page: Page, base_url: str):
        ...
        eligibility_section = page.locator(
            "text=/eligibility|criteria|requirement/i"
        ).first
...
class TestAdminVideoDemos:
    ...
    def test_demo_7_admin_create_program(self, page: Page, base_url: str):
        ...
        create_button = page.locator(
            'button:has-text("Create"), a:has-text("New Program")'
        ).first
        ...
        for _i, field in enumerate(form_fields[:5]):  # Show first 5 fields
            try:
                field_type = field.get_attribute("type") or "text"
                if field_type not in ["submit", "button", "hidden"]:
                    try:
