...
class ApplicationsPage(BasePage):
    ...
    APPLICATIONS_CONTAINER = (
        '[data-testid="applications-list"], .applications-container'
    )
    ...
    CREATE_APPLICATION_BUTTON = (
        '[data-testid="create-application"], a:has-text("Create Application")'
    )
    ...
    def get_application_titles(self) -> list[str]:
        ...
        for i in range(count):
            title = (
                self.page.locator(self.APPLICATION_CARD)
                .nth(i)
                .locator(self.APPLICATION_TITLE)
                .text_content()
            )
            if title:
