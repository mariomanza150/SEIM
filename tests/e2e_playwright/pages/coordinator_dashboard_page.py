...
class CoordinatorDashboardPage(DashboardPage):
    ...
    PENDING_APPLICATIONS_LINK = (
        '[data-testid="pending-applications"], a:has-text("Pending Applications")'
    )
    REVIEW_DOCUMENTS_LINK = (
        '[data-testid="review-documents"], a:has-text("Review Documents")'
    )
    MANAGE_PROGRAMS_LINK = (
        '[data-testid="manage-programs"], a:has-text("Manage Programs")'
    )
    ...
    # Quick actions
    REVIEW_APPLICATION_BUTTON = (
        '[data-testid="review-application"], button:has-text("Review")'
    )
    BULK_APPROVE_BUTTON = (
        '[data-testid="bulk-approve"], button:has-text("Bulk Approve")'
    )
