...
class AdminDashboardPage(DashboardPage):
    ...
    USER_MANAGEMENT_LINK = (
        '[data-testid="user-management"], a:has-text("User Management")'
    )
    SYSTEM_SETTINGS_LINK = (
        '[data-testid="system-settings"], a:has-text("System Settings")'
    )
    ...
    CREATE_PROGRAM_BUTTON = (
        '[data-testid="create-program"], button:has-text("Create Program")'
    )
    VIEW_ALL_APPLICATIONS_BUTTON = (
        '[data-testid="view-all-applications"], button:has-text("All Applications")'
    )
