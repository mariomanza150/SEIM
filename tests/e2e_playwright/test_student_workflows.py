...
from tests.e2e_playwright.utils.auth_helpers import VueAppNotAvailable, login_as_student
...
class TestStudentWorkflows:
    ...
    def test_browse_programs(self, page: Page, base_url: str):
        ...
        (
            page.locator("text=/program|exchange|apply/i").count() > 0
            or page.locator(".program, .card, table, .list-group").count() > 0
        )
    ...
    def test_view_applications_list(self, page: Page, base_url: str):
        ...
        (
            page.locator("text=/application|my applications|no applications/i").count()
            > 0
            or page.locator(
                ".application, .card, table, .list-group, .empty-state"
            ).count()
            > 0
        )
    ...
    def test_create_application_page(self, page: Page, base_url: str):
        ...
        assert "login" not in page.url.lower(), (
            "Should be able to access create application page"
        )
        ...
        (
            ...
            or page.locator("input, select, textarea").count() > 0
            or page.locator("text=/create|new application|apply/i").count() > 0
        )

        print("✅ Create application page accessible")
        page.screenshot(
            path="tests/e2e_playwright/screenshots/create_application_page.png"
        )
    ...
    def test_view_profile(self, page: Page, base_url: str):
        ...
        (
            page.locator("text=/profile|user|account/i").count() > 0
            or page.locator(".profile, .user-info, form").count() > 0
        )
