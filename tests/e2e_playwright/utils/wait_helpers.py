...
def wait_for_element(
    page: Page, selector: str, timeout: int = 30000, state: str = "visible"
) -> None:
    """
...
def wait_for_condition(
    ...
    page.wait_for_function(
        f"() => ({condition})", timeout=timeout, polling=poll_interval
    )
...
def wait_for_ajax_complete(page: Page, timeout: int = 30000) -> None:
    ...
    """
    page.wait_for_function(
        "() => typeof jQuery !== 'undefined' && jQuery.active === 0", timeout=timeout
    )
...
def wait_for_console_message(page: Page, text: str | None = None, timeout: int = 30000):
    """
    ...
    if text and text not in message.text:
        raise AssertionError(
            f"Console message '{message.text}' does not contain '{text}'"
        )
    return message
