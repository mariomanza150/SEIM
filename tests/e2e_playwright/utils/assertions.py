...
def assert_element_contains_text(
    page: Page, selector: str, text: str, timeout: int = 5000
) -> None:
    """
...
def assert_element_count(
    page: Page, selector: str, count: int, timeout: int = 5000
) -> None:
    """
...
def assert_alert_message(
    page: Page, message: str, alert_type: str | None = None, timeout: int = 5000
) -> None:
    """
...
def assert_no_errors(page: Page, timeout: int = 2000) -> None:
    ...
    try:
        expect(
            page.locator(".alert-danger, .alert-error, .error-message")
...
def assert_loading_complete(page: Page, timeout: int = 10000) -> None:
    ...
    try:
        expect(
            page.locator('.spinner, .loading, [data-testid="loading"]')
        ).to_have_count(0, timeout=timeout)
    except:
