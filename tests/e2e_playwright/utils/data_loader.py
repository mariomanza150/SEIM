...
def create_test_programs(programs_data: list[dict[str, Any]] = None) -> list[Program]:
    ...
    for program_data in programs_data:
        program, created = Program.objects.get_or_create(
            name=program_data["name"], defaults=program_data
        )
...
def create_test_applications(
    applications_data: list[dict[str, Any]] = None,
) -> list[Application]:
    """
    ...
    for app_data in applications_data:
        ...
        app, created = Application.objects.get_or_create(
            student=student, program=program, defaults={"status": status}
        )
...
def seed_database(
    load_users: bool = True, load_programs: bool = True, load_applications: bool = True
) -> dict[str, list]:
    """
    ...
    """
    result = {"users": [], "programs": [], "applications": []}
...
def create_minimal_test_data() -> dict[str, Any]:
    ...
    coordinator = User.objects.create_user(
        username="coordinator", email="coordinator@seim.edu", password="coord123"
    )
    coordinator.roles.add(coordinator_role)

    student1 = User.objects.create_user(
        username="student1", email="student1@university.edu", password="student123"
    )
    student1.roles.add(student_role)

    student2 = User.objects.create_user(
        username="student2", email="student2@university.edu", password="student123"
    )
