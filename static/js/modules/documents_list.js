...
const containerSelector = '#documentsListContainer';
const loadMoreBtnId = 'loadMoreDocumentsBtn';
...
function renderDocuments(documents, append = false) {
    ...
    for (const document of documents) {
        html += renderDocumentCard(document);
    }
    if (append) {
        ...
        if (window.SEIM_SECURITY_UTILS) {
            ...
            container.textContent = 'Documents loaded';
        }
    } else {
    ...
    <div class="col-md-6 col-lg-4">
        <div class="card h-100">
            <div class="card-header">
                ...
                <p class="text-muted mb-0">${document.type}</p>
            </div>
            <div class="card-body">
                <div class="mb-2">
                    <small class="text-muted">Uploaded</small><br>
                    <strong>${document.uploaded_at || 'N/A'}</strong>
                </div>
                <div class="mb-2">
                    <small class="text-muted">Status</small><br>
                    <strong>${document.status || 'N/A'}</strong>
                </div>
            </div>
...
async function fetchDocuments(page, append = false) {
    ...
    try {
        ...
        errorHandler.handleApiError(error, { context: 'fetchDocuments', page, filters });
...
export function initDocumentsList() {
    currentPage = 1;
    hasNextPage = true;
    filters = {};
    setupFilters();
