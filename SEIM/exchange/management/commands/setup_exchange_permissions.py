...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            coordinator_group, created = Group.objects.get_or_create(name="Exchange Coordinators")
            if created:
                self.stdout.write(self.style.SUCCESS("Created Exchange Coordinators group"))

            administrator_group, created = Group.objects.get_or_create(name="Exchange Administrators")
            if created:
                self.stdout.write(self.style.SUCCESS("Created Exchange Administrators group"))
            ...
            administrator_group.permissions.add(review_perm, approve_perm, view_all_perm)

            self.stdout.write(self.style.SUCCESS("Assigned permissions to groups"))

            # Display summary
            self.stdout.write("\n=== PERMISSION SETUP SUMMARY ===")
            self.stdout.write(f"Exchange Coordinators: {coordinator_group.permissions.count()} permissions")
            self.stdout.write(f"Exchange Administrators: {administrator_group.permissions.count()} permissions")

            self.stdout.write("\n=== NEXT STEPS ===")
            self.stdout.write("1. Add users to appropriate groups via Django admin")
            self.stdout.write("2. Or use: python manage.py assign_user_group <username> <group_name>")
