...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--exchange-id", type=int, help="Scan documents for a specific exchange ID")
        parser.add_argument(
            "--verbose",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if not scanner.scanner:
            self.stdout.write(
                self.style.WARNING("Warning: Malware scanner not available. Please install and configure ClamAV.")
            )
            return
        ...
        if options["unscanned_only"]:
            ...
            queryset = queryset.filter(models.Q(virus_scanned=False) | models.Q(virus_scan_date__lt=cutoff_date))
        ...
        for document in queryset.iterator(chunk_size=50):
            ...
            if options["verbose"]:
                self.stdout.write(f"Scanning: {document.original_filename} (ID: {document.id})")
            ...
            try:
                ...
                if options["verbose"]:
                    self.stdout.write(self.style.ERROR(f"Error scanning {document.id}: {e}"))

        # Report results
        self.stdout.write(self.style.SUCCESS(f"\nScan complete. Scanned {scanned} documents."))

        if infected_documents:
            self.stdout.write(self.style.ERROR(f"ALERT: Found {len(infected_documents)} infected documents:"))

            for doc in infected_documents:
                self.stdout.write(self.style.ERROR(f"  - {doc.original_filename} (ID: {doc.id}) - QUARANTINED"))

        if errors:
            self.stdout.write(self.style.WARNING(f"Encountered {len(errors)} errors during scanning:"))

            for doc, error in errors:
                self.stdout.write(self.style.WARNING(f"  - {doc.original_filename} (ID: {doc.id}): {error}"))
