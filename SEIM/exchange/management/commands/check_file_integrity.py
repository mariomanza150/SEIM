...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--exchange-id", type=int, help="Check documents for a specific exchange ID")
        parser.add_argument("--category", type=str, help="Check documents of a specific category")
        parser.add_argument(
            "--fix",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        for document in queryset.iterator(chunk_size=100):
            ...
            if options["verbose"]:
                self.stdout.write(f"Checking: {document.original_filename} (ID: {document.id})")
            ...
            try:
                ...
                if options["fix"]:
                    document.verification_notes = f"Integrity check failed on {timezone.now()}: {str(e)}"
                    document.is_verified = False
                    document.save()
        ...
        if corrupt_documents:
            self.stdout.write(self.style.WARNING(f"Found {len(corrupt_documents)} documents with integrity issues:"))

            for doc, issue in corrupt_documents:
                self.stdout.write(self.style.ERROR(f"  - {doc.original_filename} (ID: {doc.id}): {issue}"))

            if options["fix"]:
                self.stdout.write(
                    self.style.SUCCESS(f"Marked {len(corrupt_documents)} documents as having integrity issues.")
                )
        else:
            self.stdout.write(self.style.SUCCESS("All documents passed integrity check!"))
