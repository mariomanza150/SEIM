...
class UserProfileForm(forms.ModelForm):
    ...
    student_id = forms.CharField(required=False, widget=BootstrapWidgets.text_input(placeholder="Student ID"))
    current_program = forms.CharField(
        required=False,
        widget=BootstrapWidgets.text_input(placeholder="Current Program"),
    )
    phone = forms.CharField(required=False, widget=BootstrapWidgets.text_input(placeholder="Phone Number"))
    date_of_birth = forms.DateField(required=False, widget=BootstrapWidgets.date_input())
    gender = forms.ChoiceField(
        required=False,
        choices=UserProfile.GENDER_CHOICES,
        widget=BootstrapWidgets.select(),
    )
    address = forms.CharField(required=False, widget=BootstrapWidgets.text_input(placeholder="Street Address"))
    city = forms.CharField(required=False, widget=BootstrapWidgets.text_input(placeholder="City"))
    country = forms.CharField(required=False, widget=BootstrapWidgets.text_input(placeholder="Country"))
    ...
    def clean(self):
        """Additional form validation for field consistency."""
        cleaned_data = super().clean()
        address = cleaned_data.get("address")
        city = cleaned_data.get("city")
        country = cleaned_data.get("country")

        # Ensure address fields are filled together
        if any([address, city, country]) and not all([address, city, country]):
            raise ValidationError("Address, city, and country must all be provided together.")
    ...
    def save(self, commit=True):
        ...
        update_if_not_empty(profile, "date_of_birth", self.cleaned_data.get("date_of_birth"))
        ...
        update_if_not_empty(profile, "current_program", self.cleaned_data.get("current_program"))
