...
def exchange_list(request):
    ...
    if search_query:
        exchanges = exchanges.filter(
            ...
            | Q(host_university__icontains=search_query)
...
def exchange_detail(request, pk):
    ...
    # Get documents - already optimized if using prefetch_related
            ...
            try:
                with transaction.atomic():
                    ...
                    if action == "draft":
                        ...
                        logger.info(f"Saving exchange as draft for user {request.user.username}")
                    elif action == "submit":
                        # Validate before submission
                        exchange.status = "DRAFT"  # Temporarily set to draft for validation
                        ...
                        if not can_submit:
                            ...
                            logger.warning(f"Submission validation failed for user {request.user.username}: {reason}")
                            return render(
                                request,
                        ...
                        logger.info(f"Submitting exchange for user {request.user.username}")
                    else:
                    ...
                    success_message = (
                        f"Application {'submitted' if action == 'submit' else 'saved as draft'} successfully!"
                    )
                    messages.success(request, success_message)
                    logger.info(f"Exchange {exchange.id} created successfully by {request.user.username}")
                ...
                logger.error(f"Exchange creation failed for user {request.user.username}: {str(e)}")
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    if action == "submit":
                        messages.success(request, "Exchange application submitted successfully!")
                    else:
                        messages.success(request, "Exchange application updated successfully.")

                    return redirect("exchange:exchange-detail", pk=exchange.id)

            except Exception as e:
                logger.error(f"Exchange update failed for user {request.user.username}: {str(e)}")
    ...
    if request.method == "POST":
        ...
        if content:
            try:
                ...
                logger.info(f"Comment added to exchange {exchange.id} by user {request.user.username}")

            except Exception as e:
                logger.error(f"Failed to add comment to exchange {exchange.id}: {str(e)}")
            ...
            data.append(
                {
                    ...
                    "start_date": (exchange.start_date.isoformat() if exchange.start_date else None),
                    "end_date": (exchange.end_date.isoformat() if exchange.end_date else None),
                    "status": exchange.status,
                    "status_display": dict(Exchange.STATUS_CHOICES).get(exchange.status, exchange.status),
