...
def export_report_view(request):
    ...
    exchanges = AnalyticsService.export_exchange_data(start_date=start_date, end_date=end_date, status=status)

    # Create CSV response
    response = HttpResponse(content_type="text/csv")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    response["Content-Disposition"] = f'attachment; filename="exchange_report_{timestamp}.csv"'
    ...
    for exchange in exchanges:
        writer.writerow(
            [
                ...
                (exchange.submission_date.strftime("%Y-%m-%d") if exchange.submission_date else ""),
                (exchange.decision_date.strftime("%Y-%m-%d") if exchange.decision_date else ""),
