...
class SecureFileValidator:
    ...
    def validate_file(cls, file_obj, allowed_extensions=None):
        ...
        if file_obj.size > cls.MAX_FILE_SIZE:
            raise ValidationError(f"The file is too large. Maximum size is {cls.MAX_FILE_SIZE / (1024 * 1024)}MB.")
        ...
        try:
            ...
            for pattern in cls.SUSPICIOUS_PATTERNS:
                if re.search(pattern, file_content):
                    logger.warning(
                        f"Suspicious content detected in uploaded file: {original_filename}. " f"Pattern: {pattern}"
                    )
                    raise ValidationError("This file contains potentially unsafe content and cannot be uploaded.")
        ...
        try:
            ...
            for pattern in cls.SUSPICIOUS_PATTERNS:
                if re.search(pattern, content):
                    logger.warning(f"Suspicious content detected in file: {file_path}. " f"Pattern: {pattern}")
