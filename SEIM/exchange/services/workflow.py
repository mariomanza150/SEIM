...
class WorkflowService:
    ...
    def transition(exchange, new_status, user=None, comment=""):
        ...
        try:
            ...
            WorkflowService._post_transition_actions(exchange, old_status, new_status, user)
    ...
    def get_available_transitions(exchange, user):
        ...
        for status in exchange.TRANSITIONS.get(exchange.status, []):
            # Check permissions
            if status in ["APPROVED", "REJECTED"] and not user.has_perm("exchange.can_approve_exchange"):
                continue
            if status == "UNDER_REVIEW" and not user.has_perm("exchange.can_review_exchange"):
                continue
    ...
    def can_submit(exchange):
        ...
        if profile and not profile.student_id:
            errors.append("Student ID is required")
        elif not profile:
            errors.append("User profile is incomplete")
        ...
        for field, label in required_exchange_fields.items():
        ...
        if exchange.motivation_letter:
            ...
            if word_count < 200:
                errors.append(f"Statement of purpose must be at least 200 words (current: {word_count})")
