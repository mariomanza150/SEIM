...
class DataTableOptimizer:
    ...
    def optimize_exchange_queryset(user=None):
        ...
        queryset = (
            ...
            .prefetch_related(
                ...
                Prefetch(
                    "documents",
                    queryset=Document.objects.only("id", "status", "category", "file_size", "uploaded_at"),
                ),
            ...
            .annotate(
                ...
                pending_document_count=Count("documents", filter=Q(documents__status="PENDING")),
                last_activity=Max("timeline__created_at"),
                days_since_created=models.functions.Extract(models.functions.Now() - F("created_at"), "days"),
            )
        )
    ...
    def optimize_document_queryset(user=None):
        ...
        queryset = (
            ...
            .annotate(
                ...
                days_since_upload=models.functions.Extract(models.functions.Now() - F("uploaded_at"), "days"),
            )
        )
    ...
    def optimize_search_query(queryset, search_value, search_fields=None):
        ...
        queryset = queryset.annotate(search_rank=models.Value(1, output_field=models.IntegerField()))
    ...
    def get_dashboard_stats(user=None):
        ...
        def calculate_stats():
            ...
            stats = base_queryset.aggregate(
                ...
                avg_processing_days=Avg(
                    models.functions.Extract(F("decision_date") - F("created_at"), "days"),
        ...
        return DataTableOptimizer.get_cached_stats(cache_key, calculate_stats, timeout=600)  # 10 minutes cache
    ...
    def get_popular_destinations(limit=10, user=None):
        ...
        def calculate_destinations():
            ...
            return list(queryset.values("destination_country").annotate(count=Count("id")).order_by("-count")[:limit])

        return DataTableOptimizer.get_cached_stats(cache_key, calculate_destinations, timeout=3600)  # 1 hour cache
    ...
    def get_monthly_trends(months=12, user=None):
        ...
        def calculate_trends():
            ...
            queryset = DataTableOptimizer.optimize_exchange_queryset(user).filter(created_at__gte=start_date)
        ...
        return DataTableOptimizer.get_cached_stats(cache_key, calculate_trends, timeout=1800)  # 30 minutes cache
...
class QueryOptimizationMixin:
    ...
    def apply_search_optimization(self, queryset, search_value):
        ...
        if hasattr(self, "search_fields") and self.search_fields:
            return DataTableOptimizer.optimize_search_query(queryset, search_value, self.search_fields)
        return queryset
    ...
    def get_cached_count(self, queryset, cache_key_suffix=""):
        ...
        return DataTableOptimizer.get_cached_stats(cache_key, calculate_count, timeout=300)  # 5 minutes
