...
class ExchangeWorkflowIntegrationTest(TestCase):
    ...
    def test_complete_exchange_workflow(self):
        ...
        document_file = SimpleUploadedFile("test_document.pdf", b"file_content", content_type="application/pdf")
        ...
        self.assertTrue(any(n.title.lower().contains("approved") for n in student_notifications))
    ...
    def test_rejection_workflow(self):
        ...
        notifications = Notification.objects.filter(user=self.student, title__icontains="rejected")
        self.assertTrue(notifications.exists())

    def test_document_management_workflow(self):
        """Test document upload, review, and deletion workflow"""
        # Create exchange
        exchange = Exchange.objects.create(student=self.student, first_name="Test", last_name="Student", status="DRAFT")
        ...
        for doc_type in document_types:
            ...
            document_file = SimpleUploadedFile(f"{doc_type.lower()}.pdf", file_content, content_type="application/pdf")
    ...
    def test_filter_and_search_workflow(self):
        ...
        # Test status filter
        response = self.client.get(reverse("exchange:exchange-list"), {"status": "APPROVED"})
        ...
        # Test country filter
        response = self.client.get(reverse("exchange:exchange-list"), {"country": "Germany"})
    ...
    def test_permission_workflow(self):
        ...
        response = self.client.get(reverse("exchange:exchange-detail", args=[exchange.id]))
        self.assertEqual(response.status_code, 403)

        # Should not be able to edit
        response = self.client.get(reverse("exchange:exchange-edit", args=[exchange.id]))
        self.assertEqual(response.status_code, 403)
