...
class TestExchangeFilter:
    ...
    def test_status_filter(self, exchanges):
        ...
        filter_set = ExchangeFilter({"status": "SUBMITTED"}, queryset=Exchange.objects.all())
    ...
    def test_student_info_filters(self, exchanges):
        ...
        filter_set = ExchangeFilter({"first_name": "Jane"}, queryset=Exchange.objects.all())
        assert filter_set.qs.count() == 1
    ...
    def test_academic_filters(self, exchanges):
        ...
        # Test GPA range
        filter_set = ExchangeFilter({"gpa_min": "3.4", "gpa_max": "3.9"}, queryset=Exchange.objects.all())

        # Should get exchanges with GPA 3.5 and 3.8
        assert filter_set.qs.count() == 2

        # Test current year
        filter_set = ExchangeFilter({"current_year": 3}, queryset=Exchange.objects.all())
        assert filter_set.qs.count() == 1
    ...
    def test_destination_filters(self, exchanges):
        ...
        filter_set = ExchangeFilter({"destination_country": "USA"}, queryset=Exchange.objects.all())
        assert filter_set.qs.count() == 1
        assert filter_set.qs.first().destination_country == "USA"

        # Test destination university (partial match)
        filter_set = ExchangeFilter({"destination_university": "Cambridge"}, queryset=Exchange.objects.all())
        assert filter_set.qs.count() == 1
    ...
    def test_full_text_search(self, exchanges):
        ...
        filter_set = ExchangeFilter({"search": "Engineering"}, queryset=Exchange.objects.all())
        assert filter_set.qs.count() == 1
        assert filter_set.qs.first().current_program == "Engineering"

        # Search for email domain
        filter_set = ExchangeFilter({"search": "@example.com"}, queryset=Exchange.objects.all())
        assert filter_set.qs.count() == 3  # All have @example.com

    def test_ordering(self, exchanges):
        """Test ordering functionality."""
        # Order by GPA descending
        filter_set = ExchangeFilter({"ordering": "-gpa"}, queryset=Exchange.objects.all())
        ...
        filter_set = ExchangeFilter({"ordering": "submission_date"}, queryset=Exchange.objects.all())
        ordered_qs = list(filter_set.qs)
