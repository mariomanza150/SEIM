...
class TestEmailService:
    ...
    def test_send_email_via_smtp(self, mock_strip_tags, mock_render_to_string):
        ...
        with override_settings(USE_AWS_SES=False, DEFAULT_FROM_EMAIL="noreply@test.com"):
            with patch("exchange.services.email_service.EmailMultiAlternatives") as mock_email:
                ...
                mock_render_to_string.assert_called_once_with("email/test_template.html", {"key": "value"})
                ...
                mock_email_instance.attach_alternative.assert_called_once_with("<html>Test HTML</html>", "text/html")
    ...
    def test_send_email_error_handling(self):
        ...
        with override_settings(USE_AWS_SES=False):
            with patch("exchange.services.email_service.render_to_string") as mock_render:
...
class TestExchangeEmailNotificationService:
    ...
    @patch.object(EmailService, "send_email")
    def test_notify_review_assigned(self, mock_send_email, notification_service, exchange, user):
    ...
    @patch.object(EmailService, "send_email")
    def test_notify_document_uploaded_draft_status(self, mock_send_email, notification_service, exchange):
        """Test that no notification is sent for document upload when status is DRAFT."""
        result = notification_service.notify_document_uploaded(exchange, "test_document.pdf")

        assert result is True
        mock_send_email.assert_not_called()

    @patch.object(EmailService, "send_email")
    def test_notify_document_uploaded_submitted_status(self, mock_send_email, notification_service, exchange):
