...
class GradeScale(UUIDModel, TimeStampedModel):
    ...
    description = models.TextField(
        blank=True, help_text="Detailed description of the grading scale"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether this scale is currently in use"
    )
...
class GradeValue(UUIDModel, TimeStampedModel):
    ...
    label = models.CharField(
        max_length=50, help_text="Grade label (e.g., 'A', '1.0', 'First Class')"
    )
    ...
    description = models.TextField(
        blank=True, help_text="Description of what this grade represents"
    )
    order = models.PositiveIntegerField(
        default=0, help_text="Display order within the scale"
    )
    is_passing = models.BooleanField(
        default=True, help_text="Whether this grade is considered passing"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["grade_scale", "gpa_equivalent"], name="grade_val_gpa_idx"
            ),
            models.Index(
                fields=["grade_scale", "numeric_value"], name="grade_val_num_idx"
            ),
        ]
    ...
    def clean(self):
        ...
        if self.grade_scale:
            if (
                self.numeric_value < self.grade_scale.min_value
                or self.numeric_value > self.grade_scale.max_value
            ):
                raise ValidationError(
                    {
...
class GradeTranslation(UUIDModel, TimeStampedModel):
    ...
    def __str__(self):
        return (
            f"{self.source_grade.label} ({self.source_grade.grade_scale.code}) → "
