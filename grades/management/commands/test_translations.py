...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write("\n" + "=" * 60)
        self.stdout.write(
            self.style.MIGRATE_HEADING("GRADE TRANSLATION SERVICE VERIFICATION")
        )
        ...
        self.stdout.write(
            f"German 1.3 (GPA {german_1_3.gpa_equivalent}) -> US {result.label} (GPA {result.gpa_equivalent})"
        )
        ...
        self.stdout.write(
            f"GPA 3.5 -> ECTS {result.label} (GPA {result.gpa_equivalent})"
        )
        ...
        self.stdout.write(
            f"Student: {result['student_grade']} = {result['student_gpa_equivalent']} GPA"
        )
        self.stdout.write(
            f"Required: {result['required_grade']} = {result['required_gpa_equivalent']} GPA"
        )
        ...
        assert not result["eligible"], (
            f"Expected eligible=False, got {result['eligible']}"
        )
        self.stdout.write(self.style.SUCCESS("✅ PASSED"))
        ...
        translation = GradeTranslationService.suggest_translation(
            str(us_a.id), str(ects_a.id), notes="Test translation"
        )
        self.stdout.write(
            f"Created translation: US {us_a.label} -> ECTS {ects_a.label}"
        )
        self.stdout.write(f"Confidence: {translation.confidence:.2f}")
        assert translation.confidence == 1.0, (
            f"Expected confidence 1.0, got {translation.confidence}"
        )
        self.stdout.write(self.style.SUCCESS("✅ PASSED"))
        ...
        translations = GradeTranslationService.bulk_create_translations(
            str(us.id), str(ects.id), mapping
        )
        self.stdout.write(f"Created {len(translations)} translations")
        assert len(translations) == 2, (
            f"Expected 2 translations, got {len(translations)}"
        )
        self.stdout.write(self.style.SUCCESS("✅ PASSED"))
