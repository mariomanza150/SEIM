...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write("\n" + "=" * 60)
        self.stdout.write(
            self.style.MIGRATE_HEADING("GRADE TRANSLATION INTEGRATION VERIFICATION")
        )
        ...
        assert "grades.GradeTranslation" in registered_models, (
            "GradeTranslation not registered"
        )
        ...
        try:
            ...
            self.stdout.write(
                "✓ German student (1.3 = 3.7 GPA) eligible for 3.0 GPA program"
            )
            self.stdout.write(self.style.SUCCESS("✅ PASSED"))
        ...
        try:
            ApplicationService.check_eligibility(user, program)
            self.stdout.write(
                self.style.ERROR("❌ FAILED: Should have raised ValueError")
            )
