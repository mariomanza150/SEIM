...
def send_exchange_notification(
    exchange_id, notification_type, user_id=None, extra_context=None
):
    """
    ...
    try:
        ...
        if notification_type == "submission":
            ...
            result = notification_service.notify_comment_added(
                exchange, comment_author, comment_text
            )
        ...
        if result:
            logger.info(
                f"Successfully sent {notification_type} notification for exchange {exchange_id}"
            )
        else:
            logger.error(
                f"Failed to send {notification_type} notification for exchange {exchange_id}"
            )
...
def check_exchange_deadlines():
    ...
    try:
        ...
        exchanges_starting_soon = Exchange.objects.filter(
            status="APPROVED", start_date=start_soon_date
        )
        ...
        incomplete_exchanges = Exchange.objects.filter(
            status="DRAFT", created_at__lte=deadline_date
        )
        ...
        logger.info(
            f"Processed {exchanges_starting_soon.count()} exchanges starting soon"
        )
        logger.info(
            f"Sent {incomplete_exchanges.count()} application deadline reminders"
        )
    ...
    try:
        ...
        if notification_service.admin_emails:
            notification_service.email_service.send_email(
                ...
                attachments=[
                    ("exchange_report.pdf", report["pdf_content"], "application/pdf")
                ],
            )
