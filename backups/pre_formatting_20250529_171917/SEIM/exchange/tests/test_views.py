...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.coordinator_profile = UserProfile.objects.create(
            user=self.coordinator_user, role="COORDINATOR"
        )
        ...
        self.admin_profile = UserProfile.objects.create(
            user=self.admin_user, role="ADMINISTRATOR"
        )
...
class DashboardViewTest(BaseTestCase):
    ...
    def test_coordinator_dashboard(self):
        ...
        self.client.login(username="coordinator1", password="testpass123")
        response = self.client.get(reverse("exchange:dashboard"))
        self.assertEqual(response.status_code, 200)
...
class ExchangeListViewTest(BaseTestCase):
    ...
    def test_student_sees_own_exchanges(self):
        ...
        other_student = User.objects.create_user(
            "student2", "student2@test.com", "testpass123"
        )
    ...
    def test_filter_by_status(self):
        ...
        response = self.client.get(
            reverse("exchange:exchange-list") + "?status=SUBMITTED"
        )
...
class ExchangeDetailViewTest(BaseTestCase):
    ...
    def test_detail_view_requires_login(self):
        ...
        response = self.client.get(
            reverse("exchange:exchange-detail", args=[self.exchange.id])
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_student_cannot_view_others_exchange(self):
        ...
        UserProfile.objects.create(user=other_student, role="STUDENT")
        other_exchange = Exchange.objects.create(
            student=other_student, first_name="Other", last_name="Student"
        )

        self.client.login(username="student1", password="testpass123")
        response = self.client.get(
            reverse("exchange:exchange-detail", args=[other_exchange.id])
        )
...
class ExchangeCreateViewTest(BaseTestCase):
    ...
    def test_create_exchange_post_valid(self):
        ...
        self.assertRedirects(
            response, reverse("exchange:exchange-detail", args=[new_exchange.id])
        )
    ...
    def test_create_exchange_save_draft(self):
        ...
        draft_exchange = Exchange.objects.filter(
            student=self.student_user, first_name="Draft"
        ).first()
...
class ExchangeEditViewTest(BaseTestCase):
    ...
    def test_edit_requires_login(self):
        ...
        response = self.client.get(
            reverse("exchange:exchange-edit", args=[self.exchange.id])
        )
        self.assertEqual(response.status_code, 302)
