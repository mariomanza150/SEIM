...
class AuthenticationTestCase(TestCase):
    ...
    def test_login(self):
        ...
        response = self.client.post(
            "/api/auth/login/", {"username": "student1", "password": "testpass123"}
        )
...
class PermissionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.student1_profile = UserProfile.objects.create(
            user=self.student1, role="STUDENT"
        )
        ...
        self.student2_profile = UserProfile.objects.create(
            user=self.student2, role="STUDENT"
        )
        self.student2_token = Token.objects.create(user=self.student2)

        self.coordinator = User.objects.create_user(
            username="coord1", password="test123", is_staff=True
        )
        self.coordinator_profile = UserProfile.objects.create(
    ...
    def test_coordinator_can_view_all_exchanges(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Token {self.coordinator_token.key}"
        )
