...
def upload_document(request, pk):
    ...
    if request.method == "POST":
        ...
        # Check permission
        if exchange.student != request.user and not request.user.has_perm(
            "exchange.can_upload_any_document"
        ):
            messages.error(
                request,
    ...
    if exchange.student != request.user and not request.user.has_perm(
        ...
        messages.error(
            request, "You don't have permission to upload documents for this exchange."
        )
        return redirect("exchange:exchange-detail", pk=pk)
...
def document_list(request, pk):
    ...
    if not (
        ...
        or request.user.has_perm("exchange.can_review_exchange")
    ):
...
def download_document(request, pk):
    ...
    # Check permission
    if not (
        document.exchange.student == request.user
        or request.user.has_perm("exchange.can_review_exchange")
    ):
    ...
    if not SecureFileValidator.verify_file_integrity(document):
        ...
        return redirect(
            "exchange:document-detail", pk=document.exchange.id, doc_id=document.id
        )
