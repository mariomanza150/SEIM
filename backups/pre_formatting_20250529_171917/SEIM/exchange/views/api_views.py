...
class ExchangeDataTableView(View):
    ...
    def post(self, request):
        try:
            ...
            advanced_filters = request.GET.get("advanced_filters") or request.POST.get(
                "advanced_filters"
            )
            if advanced_filters:
                try:
            ...
            for exchange in queryset:
                row = [
                    ...
                    self.get_status_badge(
                        exchange.status, exchange.get_status_display()
                    ),
    ...
    def post(self, request):
        try:
            ...
            if action_type == "reject" and not comment.strip():
                return JsonResponse(
                    {"error": "Comment is required for rejection"}, status=400
                )
            ...
            exchanges = Exchange.objects.filter(
                id__in=valid_ids, status__in=["SUBMITTED", "UNDER_REVIEW"]
            )

            if not exchanges.exists():
                return JsonResponse(
                    {"error": "No valid exchanges found for bulk action"}, status=400
                )
            ...
            for exchange in exchanges:
                try:
                    ...
                    try:
                        ...
                        print(
                            f"Email notification failed for exchange {exchange.id}: {email_error}"
                        )
        ...
        try:
            ...
            for document in queryset:
                row = [
                    ...
                    self.get_document_status_badge(
...
class PendingApprovalsDataTableView(View):
    ...
    def get(self, request):
        try:
            ...
            queryset = Exchange.objects.select_related("student").filter(
            ...
            # Record counts
            records_total = Exchange.objects.filter(
                status__in=["SUBMITTED", "UNDER_REVIEW"]
            ).count()
            records_filtered = queryset.count()
        ...
        try:
            ...
            for timeline in queryset:
                row = [
                    ...
                    (
                        timeline.created_by.get_full_name()
