...
def batch_document_verification(request):
    ...
    if not documents.exists():
        messages.warning(
            request, f"No pending documents found in category: {document_category}"
        )
        return redirect("exchange:batch-processing")
...
def import_csv(request):
    ...
    # Process import
    results = BatchProcessor.import_exchanges_from_csv(
        csv_file=csv_file, user=request.user
    )
...
def download_csv_template(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="exchange_import_template_{timestamp}.csv"'
    )
...
def batch_notifications(request):
    ...
    if not exchanges.exists():
        messages.warning(
            request, f"No exchanges found with status: {notification_status}"
        )
        return redirect("exchange:batch-processing")
