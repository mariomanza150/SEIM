...
def submit_exchange(request, pk):
    ...
    if exchange.student != request.user:
        ...
        logger.warning(
            f"User {request.user.username} attempted to submit exchange {pk} without permission"
        )
    ...
    try:
        with transaction.atomic():
            ...
            if not can_submit:
                ...
                logger.warning(
                    f"Submission validation failed for exchange {pk}: {reason}"
                )
            ...
            if success:
                ...
                logger.info(
                    f"Exchange {pk} submitted successfully by user {request.user.username}"
                )
            else:
...
def review_exchange(request, pk):
    ...
    if not user_can_review_exchange(request.user):
        ...
        logger.warning(
            f"User {request.user.username} attempted to review exchange {pk} without permission"
        )
        return redirect("exchange:exchange-detail", pk=exchange.id)
    ...
    try:
        with transaction.atomic():
            ...
            if success:
                ...
                logger.info(
                    f"Exchange {pk} marked under review by user {request.user.username}"
                )
            else:
        ...
        messages.error(
            request, "An error occurred while updating the application status."
        )
...
def approve_exchange(request, pk):
    ...
    if not user_can_approve_exchange(request.user):
        ...
        logger.warning(
            f"User {request.user.username} attempted to approve exchange {pk} without permission"
        )
        return redirect("exchange:exchange-detail", pk=exchange.id)
    ...
    if not exchange.can_transition_to("APPROVED"):
        ...
        logger.warning(
            f"Invalid approval attempt for exchange {pk} in status {exchange.status}"
        )
        return redirect("exchange:exchange-detail", pk=exchange.id)
        ...
        with transaction.atomic():
            ...
            if success:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to send approval notification for exchange {pk}: {str(e)}"
                    )
...
def reject_exchange(request, pk):
    ...
    if not user_can_approve_exchange(request.user):
        ...
        logger.warning(
            f"User {request.user.username} attempted to reject exchange {pk} without permission"
        )
        return redirect("exchange:exchange-detail", pk=exchange.id)
    ...
    if not exchange.can_transition_to("REJECTED"):
        ...
        logger.warning(
            f"Invalid rejection attempt for exchange {pk} in status {exchange.status}"
        )
        return redirect("exchange:exchange-detail", pk=exchange.id)
        ...
        with transaction.atomic():
            ...
            if success:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to send rejection notification for exchange {pk}: {str(e)}"
                    )
...
def complete_exchange(request, pk):
    ...
    if not user_can_approve_exchange(request.user):
        ...
        logger.warning(
            f"User {request.user.username} attempted to complete exchange {pk} without permission"
        )
        return redirect("exchange:exchange-detail", pk=exchange.id)
    ...
    if not exchange.can_transition_to("COMPLETED"):
        ...
        logger.warning(
            f"Invalid completion attempt for exchange {pk} in status {exchange.status}"
        )
        return redirect("exchange:exchange-detail", pk=exchange.id)
