...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                if invalid_exchanges.exists():
                    self.stdout.write(
                        f"Found {invalid_exchanges.count()} exchanges with missing submission dates"
                    )
                    ...
                    if not dry_run:
                        for exchange in invalid_exchanges:
                            if exchange.created_at:
                                ...
                                fixes_made += 1
                                self.stdout.write(
                                    f"  Fixed submission date for exchange {exchange.id}"
                                )
                ...
                if exchanges_missing_data.exists():
                    self.stdout.write(
                        f"Found {exchanges_missing_data.count()} exchanges with missing personal data"
                    )
                        ...
                        for exchange in exchanges_missing_data:
                            if exchange.student:
                                ...
                                if not exchange.first_name:
                                    exchange.first_name = (
                                        exchange.student.first_name or "Unknown"
                                    )
                                    updated = True
                                if not exchange.last_name:
                                    exchange.last_name = (
                                        exchange.student.last_name or "Unknown"
                                    )
                                    updated = True
                                if not exchange.email:
                                    exchange.email = (
                                        exchange.student.email or "unknown@example.com"
                                    )
                                    updated = True
                                ...
                                if updated:
                                    ...
                                    fixes_made += 1
                                    self.stdout.write(
                                        f"  Fixed basic data for exchange {exchange.id}"
                                    )
                ...
                if invalid_date_exchanges.exists():
                    self.stdout.write(
                        f"Found {invalid_date_exchanges.count()} exchanges with invalid date ranges"
                    )
                    ...
                    if not dry_run:
                        for exchange in invalid_date_exchanges:
                            ...
                            if exchange.start_date:
                                exchange.end_date = (
                                    exchange.start_date + datetime.timedelta(days=180)
                                )
                                exchange.save()
                                fixes_made += 1
                                self.stdout.write(
                                    f"  Fixed date range for exchange {exchange.id}"
                                )
                ...
                if exchanges_missing_student_number.exists():
                    ...
                    if not dry_run:
                        for exchange in exchanges_missing_student_number:
                            if exchange.student and hasattr(
                                exchange.student, "profile"
                            ):
                                if exchange.student.profile.student_id:
                                    exchange.student_number = (
                                        exchange.student.profile.student_id
                                    )
                                    exchange.save()
                                    fixes_made += 1
                                    self.stdout.write(
                                        f"  Fixed student number for exchange {exchange.id}"
                                    )
                            else:
                                ...
                                fixes_made += 1
                                self.stdout.write(
                                    f"  Generated student number for exchange {exchange.id}"
                                )

                # Fix 5: Exchanges in draft status but with submission dates
                draft_with_submission = Exchange.objects.filter(
                    status="DRAFT", submission_date__isnull=False
                )

                if draft_with_submission.exists():
                    self.stdout.write(
                        f"Found {draft_with_submission.count()} draft exchanges with submission dates"
                    )
                    ...
                    if not dry_run:
                        for exchange in draft_with_submission:
                            ...
                            fixes_made += 1
                            self.stdout.write(
                                f"  Cleared submission date for draft exchange {exchange.id}"
                            )
                ...
                if dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Exchange data cleanup completed - {fixes_made} fixes applied"
                        )
                    )
        ...
        self.stdout.write(
            "3. Consider running this command periodically for maintenance"
        )
