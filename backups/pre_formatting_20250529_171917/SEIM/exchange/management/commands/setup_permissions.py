        ...
        try:
            ...
            def get_permission(codename, content_type):
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Permission {codename} does not exist for {content_type.model}"
                        )
                    )
                    return None
            ...
            # Create groups
            if not dry_run:
                student_group, student_created = Group.objects.get_or_create(
                    name="Students"
                )
                coordinator_group, coordinator_created = Group.objects.get_or_create(
                    name="Coordinators"
                )
                manager_group, manager_created = Group.objects.get_or_create(
                    name="Managers"
                )
                ...
                self.stdout.write(
                    "Would create/find groups: Students, Coordinators, Managers, Administrators"
                )
            ...
            self.stdout.write(
                f"Coordinator permissions: {len(coordinator_perms)} permissions"
            )
            ...
            all_perms = Permission.objects.filter(
                content_type__in=[exchange_ct, document_ct, course_ct, grade_ct]
            )

            self.stdout.write(
                f"Administrator permissions: {all_perms.count()} permissions"
            )
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    f"Students: {student_group.permissions.count()} permissions"
                )
                self.stdout.write(
                    f"Coordinators: {coordinator_group.permissions.count()} permissions"
                )
                self.stdout.write(
                    f"Managers: {manager_group.permissions.count()} permissions"
                )
                self.stdout.write(
                ...
                for perm in student_group.permissions.all():
                    self.stdout.write(
                        f"  - {perm.content_type.model}.{perm.codename}: {perm.name}"
                    )

                self.stdout.write("\nCoordinators (additional to Students):")
                coordinator_additional = set(coordinator_group.permissions.all()) - set(
                ...
                manager_additional = set(manager_group.permissions.all()) - set(
                    coordinator_group.permissions.all()
                )
