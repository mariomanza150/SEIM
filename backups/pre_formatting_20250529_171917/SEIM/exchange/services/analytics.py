...
class AnalyticsService:
    ...
    def get_exchange_status_summary():
        ...
        """
        status_counts = (
            Exchange.objects.values("status")
            .annotate(count=Count("id"))
            .order_by("status")
        )

        # Format into dictionary for easy use in templates
        result = {"labels": [], "data": [], "total": 0}

        status_dict = dict(Exchange.STATUS_CHOICES)
        for item in status_counts:
            status_display = status_dict.get(item["status"], item["status"])
            result["labels"].append(status_display)
    ...
    def get_applications_by_university(limit=10):
        ...
        """
        university_counts = (
            Exchange.objects.values("destination_university")
            .annotate(count=Count("id"))
    ...
    def get_applications_by_country(limit=10):
        ...
        """
        country_counts = (
            Exchange.objects.values("destination_country")
            .annotate(count=Count("id"))
            .order_by("-count")[:limit]
        )
    ...
    def get_processing_time_metrics():
        ...
        exchanges = Exchange.objects.filter(
            submission_date__isnull=False, decision_date__isnull=False
        ).annotate(
        ...
        # Calculate days
        processing_days = [
            (ex.decision_date - ex.submission_date).days for ex in exchanges
        ]

        return {
            "avg_days": (
                sum(processing_days) / len(processing_days) if processing_days else 0
            ),
    ...
    def get_document_statistics():
        ...
        result = {
            ...
            "verification_rate": (
                (verified_docs / total_docs * 100) if total_docs > 0 else 0
            ),
            "categories": {
                "labels": [],
                "data": [],
    ...
    def generate_activity_report(days=30):
        ...
        # Get status changes
        status_changes = WorkflowLog.objects.filter(
