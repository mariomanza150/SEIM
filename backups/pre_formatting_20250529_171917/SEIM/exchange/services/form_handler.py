    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_config in self.step.fields:
            ...
            if field_type == "text":
                ...
                choices = [
                    (c["value"], c["label"]) for c in field_config.get("choices", [])
                ]
                field = forms.ChoiceField(
                    label=field_label, required=required, choices=choices
                )
...
class FormService:
    ...
    def save_step_data(exchange, step, data, is_complete=False):
        ...
        submission, created = FormSubmission.objects.get_or_create(
            exchange=exchange, step=step, defaults={"data": {}}
        )
    ...
    def get_next_step(current_step):
        ...
        return (
            FormStep.objects.filter(order__gt=current_step.order, is_active=True)
            .order_by("order")
            .first()
        )
    ...
    def get_previous_step(current_step):
        ...
        return (
            FormStep.objects.filter(order__lt=current_step.order, is_active=True)
            .order_by("-order")
            .first()
        )
