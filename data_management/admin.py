...
class PermissionFilteredAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser:
            return qs
        if self.permission_model_name and _has_view_permission(
            request.user, self.permission_model_name
        ):
            return qs
        return qs.none()
...
class DataOperationLogAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser:
            return qs
        if DataPermission.objects.filter(
            role__in=request.user.roles.all(), is_active=True
        ).exists():
