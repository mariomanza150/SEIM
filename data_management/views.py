...
class BulkOperationView(View):
    ...
    def get(self, request):
        if not has_data_permission(request.user, "bulk_operation", "VIEW"):
            return _deny_and_redirect(
                request, "You don't have permission to view bulk operations"
            )

        operations = BulkOperation.objects.filter(is_active=True)
        return render(
            request, "data_management/bulk_operations.html", {"operations": operations}
        )
    ...
    def post(self, request, operation_id):
        ...
        operation = get_object_or_404(
            BulkOperation.objects.filter(is_active=True), id=operation_id
        )
        _queue_operation(
            user=request.user,
...
class DataExportView(View):
    ...
    def get(self, request):
        if not has_data_permission(request.user, "data_export", "VIEW"):
            return _deny_and_redirect(
                request, "You don't have permission to view data exports"
            )

        exports = DataExport.objects.filter(is_active=True)
        return render(
            request, "data_management/data_exports.html", {"exports": exports}
        )
    ...
    def post(self, request, export_id):
        ...
        export_config = get_object_or_404(
            DataExport.objects.filter(is_active=True), id=export_id
        )
        _queue_operation(
            user=request.user,
...
class DataImportView(View):
    ...
    def get(self, request):
        if not has_data_permission(request.user, "data_import", "VIEW"):
            return _deny_and_redirect(
                request, "You don't have permission to view data imports"
            )

        imports = DataImport.objects.filter(is_active=True)
        return render(
            request, "data_management/data_imports.html", {"imports": imports}
        )
    ...
    def post(self, request, import_id):
        ...
        import_config = get_object_or_404(
            DataImport.objects.filter(is_active=True), id=import_id
        )
...
class DemoDataSetupView(View):
    ...
    def get(self, request):
        if not has_data_permission(request.user, "demo_data", "VIEW"):
            return _deny_and_redirect(
                request, "You don't have permission to view demo data setup"
            )

        datasets = DemoDataSet.objects.filter(is_active=True)
        return render(
            request, "data_management/demo_data_setup.html", {"datasets": datasets}
        )
    ...
    def post(self, request, dataset_id):
        ...
        dataset = get_object_or_404(
            DemoDataSet.objects.filter(is_active=True), id=dataset_id
        )
        _queue_operation(
            user=request.user,
...
class DatabaseResetView(View):
    ...
    def get(self, request):
        if not has_data_permission(request.user, "database", "VIEW"):
            return _deny_and_redirect(
                request, "You don't have permission to view database reset"
            )
...
class DataCleanupView(View):
    ...
    def get(self, request):
        if not has_data_permission(request.user, "data_cleanup", "VIEW"):
            return _deny_and_redirect(
                request, "You don't have permission to view data cleanup"
            )
