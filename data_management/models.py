...
class DataOperationLog(UUIDModel, TimeStampedModel):
    ...
    def __str__(self):
        return (
            f"{self.get_operation_type_display()} - {self.model_name} - {self.status}"
        )
...
class BulkOperation(UUIDModel, TimeStampedModel):
    ...
    allowed_roles = models.JSONField(
        default=list
    )  # List of role names allowed to perform
...
class DemoDataSet(UUIDModel, TimeStampedModel):
    ...
    data_config = models.JSONField(
        default=dict
    )  # Configuration for generating demo data
    is_active = models.BooleanField(default=True)
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        related_name="demo_data_sets",
    )
...
class DataPermission(UUIDModel, TimeStampedModel):
    ...
    role = models.ForeignKey(
        "accounts.Role", on_delete=models.CASCADE, related_name="data_permissions"
    )
    model_name = models.CharField(max_length=100)
    permission_type = models.CharField(max_length=10, choices=PERMISSION_TYPES)
    custom_filters = models.JSONField(
        default=dict
    )  # Custom filters for this permission
    is_active = models.BooleanField(default=True)
