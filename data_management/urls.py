...
urlpatterns = [
    ...
    path(
        "bulk-operations/execute/<uuid:operation_id>/",
        BulkOperationView.as_view(),
        name="execute_bulk_operation",
    ),
    # Data export
    path("data-export/", DataExportView.as_view(), name="data_exports"),
    path(
        "data-export/execute/<uuid:export_id>/",
    ...
    path(
        "data-import/execute/<uuid:import_id>/",
    ...
    path(
        "demo-data-setup/execute/<uuid:dataset_id>/",
        DemoDataSetupView.as_view(),
        name="execute_demo_setup",
    ),
    # Database reset
    path("database-reset/", DatabaseResetView.as_view(), name="database_reset"),
    path(
        "database-reset/execute/",
        DatabaseResetView.as_view(),
        name="execute_database_reset",
    ),
    # Data cleanup
    path("data-cleanup/", DataCleanupView.as_view(), name="data_cleanup"),
    path(
        "data-cleanup/execute/", DataCleanupView.as_view(), name="execute_data_cleanup"
    ),
]
