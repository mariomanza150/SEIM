...
class FormTypeSerializer(serializers.ModelSerializer):
    ...
    required_fields = serializers.ListField(
        source="get_required_fields", read_only=True
    )
...
class FormSubmissionSerializer(serializers.ModelSerializer):
    ...
    form_type_name = serializers.CharField(source="form_type.name", read_only=True)
    submitted_by_username = serializers.CharField(
        source="submitted_by.username", read_only=True
    )
    program_name = serializers.CharField(
        source="program.name", read_only=True, allow_null=True
    )
    response_count = serializers.IntegerField(
        source="get_response_count", read_only=True
    )
...
class ApplyStepTemplateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if (
            has_id
            and not FormStepTemplate.objects.filter(pk=attrs["template_id"]).exists()
        ):
            raise serializers.ValidationError({"template_id": "Unknown step template."})
        if (
            has_slug
            and not FormStepTemplate.objects.filter(slug=attrs["slug"]).exists()
        ):
