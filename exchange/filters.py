...
class ProgramFilter(django_filters.FilterSet):
    ...
    start_date_after = django_filters.DateFilter(
        field_name="start_date", lookup_expr="gte", label="Start After"
    )
    start_date_before = django_filters.DateFilter(
        field_name="start_date", lookup_expr="lte", label="Start Before"
    )
    end_date_after = django_filters.DateFilter(
        field_name="end_date", lookup_expr="gte", label="End After"
    )
    end_date_before = django_filters.DateFilter(
        field_name="end_date", lookup_expr="lte", label="End Before"
    )

    # Eligibility filters
    min_gpa_max = django_filters.NumberFilter(
        field_name="min_gpa", lookup_expr="lte", label="Max GPA Requirement"
    )
    min_gpa_min = django_filters.NumberFilter(
        field_name="min_gpa", lookup_expr="gte", label="Min GPA Requirement"
    )
    required_language = django_filters.CharFilter(
        lookup_expr="icontains", label="Required Language"
    )
    ...
    # Age filters
    max_age_max = django_filters.NumberFilter(
        field_name="max_age", lookup_expr="lte", label="Max Age Limit"
    )
    min_age_min = django_filters.NumberFilter(
        field_name="min_age", lookup_expr="gte", label="Min Age Limit"
    )
...
class ApplicationFilter(django_filters.FilterSet):
    ...
    student_name = django_filters.CharFilter(
        method="filter_student_name", label="Student Name"
    )
    student_email = django_filters.CharFilter(
        method="filter_student_email", label="Student Email"
    )
    student_id = django_filters.UUIDFilter(field_name="student__id", label="Student ID")

    # Program filters
    program_name = django_filters.CharFilter(
        field_name="program__name", lookup_expr="icontains", label="Program Name"
    )
    ...
    status = django_filters.CharFilter(
        field_name="status__name", lookup_expr="iexact", label="Status"
    )
    status_name = django_filters.CharFilter(
        field_name="status__name", lookup_expr="iexact", label="Status Name"
    )
    ...
    active = django_filters.BooleanFilter(
        method="filter_active", label="Active (Not Withdrawn)"
    )
