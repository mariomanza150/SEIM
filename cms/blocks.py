...
class FAQItemBlock(blocks.StructBlock):
    ...
    answer = blocks.RichTextBlock(
        required=True, features=["bold", "italic", "link", "ol", "ul"]
    )
...
class FAQBlock(blocks.StructBlock):
    ...
    heading = blocks.CharBlock(
        required=False, max_length=100, default="Frequently Asked Questions"
    )
...
class EmbedVideoBlock(blocks.StructBlock):
    ...
    video = EmbedBlock(
        required=True, help_text="YouTube, Vimeo, or other supported video URL"
    )
    caption = blocks.CharBlock(required=False, max_length=250)
...
class FormBlock(blocks.StructBlock):
    ...
    form_page = blocks.PageChooserBlock(
        required=True, page_type="cms.FormPage", help_text="Select a form page to embed"
    )
