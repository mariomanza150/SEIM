...
class InternationalHomePage(SeoMixin, Page):
    ...
    # Introduction
    introduction = RichTextField(blank=True, help_text="Page introduction")
    ...
    stat_programs_count = models.IntegerField(
        default=0, help_text="Number of active programs"
    )
    stat_countries_count = models.IntegerField(
        default=0, help_text="Number of partner countries"
    )
    stat_students_count = models.IntegerField(
        default=0, help_text="Number of students in exchange"
    )
    stat_institutions_count = models.IntegerField(
        default=0, help_text="Number of partner institutions"
    )
...
class CGRIPage(SeoMixin, Page):
    ...
    subtitle = models.CharField(
        max_length=200, blank=True, help_text="Page subtitle/tagline"
    )

    introduction = models.TextField(
        max_length=500, blank=True, help_text="Page introduction"
    )
    ...
    body = StreamField(
        BaseStreamBlock(), use_json_field=True, help_text="Main content area"
    )

    # Contact Information
    show_contact = models.BooleanField(
        default=False, help_text="Show contact information sidebar"
    )
...
class MovilidadLandingPage(SeoMixin, Page):
    ...
    hero_title = models.CharField(
        max_length=200, default="Movilidad Estudiantil", help_text="Hero title"
    )
    hero_subtitle = models.TextField(
        max_length=500,
    ...
    introduction = RichTextField(blank=True, help_text="Introduction text")
    ...
    show_quick_links = models.BooleanField(
        default=True, help_text="Show quick access links"
    )

    # CTA to SEIM
    show_application_cta = models.BooleanField(
        default=True, help_text="Show application CTA linking to SEIM"
    )
    application_cta_text = models.CharField(
        max_length=100, default="Aplicar Ahora", blank=True
    )
...
class ConvenioPage(SeoMixin, Page):
    ...
    institution_name = models.CharField(
        max_length=200, help_text="Partner institution name"
    )
    ...
    start_date = models.DateField(
        null=True, blank=True, help_text="Agreement start date"
    )
    end_date = models.DateField(
        null=True, blank=True, help_text="Agreement end date (leave blank if ongoing)"
    )

    introduction = models.TextField(
        max_length=500, blank=True, help_text="Brief description"
    )
    ...
    related_programs = ParentalManyToManyField(
        "exchange.Program", blank=True, related_name="convenio_pages"
    )
...
class TestimonialPage(SeoMixin, Page):
    ...
    student_major = models.CharField(
        max_length=200, blank=True, help_text="Student's major/field of study"
    )
    exchange_period = models.CharField(
        max_length=100, blank=True, help_text="e.g., Fall 2024, Spring 2025"
    )
    ...
    quote = models.TextField(
        max_length=500, help_text="Short quote/excerpt for listings"
    )

    body = StreamField(
        BaseStreamBlock(), use_json_field=True, help_text="Full testimonial story"
    )
    ...
    would_recommend = models.BooleanField(
        default=True, help_text="Would recommend this program"
    )
...
class TestimonialIndexPage(SeoMixin, Page):
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        testimonials = (
            TestimonialPage.objects.live().public().order_by("-first_published_at")
        )
...
class HomePage(SeoMixin, Page):
    ...
    # Hero Section
    hero_title = models.CharField(
        max_length=200, default="Student Exchange Information Manager"
    )
    hero_subtitle = models.TextField(
        max_length=500, default="Streamline your student exchange program management"
    )
    hero_image = models.ForeignKey(
        "wagtailimages.Image",
        null=True,
    ...
    hero_cta_text = models.CharField(max_length=50, default="Get Started", blank=True)
...
class StandardPage(SeoMixin, Page):
    ...
    subtitle = models.CharField(
        max_length=200, blank=True, help_text="Optional subtitle below the title"
    )

    introduction = models.TextField(
        max_length=500, blank=True, help_text="Page introduction/summary"
    )
    ...
    related_pages = ParentalManyToManyField(
        "wagtailcore.Page", blank=True, related_name="+"
    )
...
class BlogPostTag(TaggedItemBase):
    ...
    content_object = ParentalKey(
        "cms.BlogPostPage", related_name="tagged_items", on_delete=models.CASCADE
    )
...
class BlogPostPage(SeoMixin, Page):
    ...
    introduction = models.TextField(max_length=500, help_text="Post summary/excerpt")

    body = StreamField(BaseStreamBlock(), use_json_field=True, help_text="Post content")

    categories = ParentalManyToManyField(
        "cms.BlogCategory", blank=True, related_name="posts"
    )
...
class ProgramPage(SeoMixin, Page):
    ...
    introduction = models.TextField(max_length=500, help_text="Program summary")

    body = StreamField(
        BaseStreamBlock(), use_json_field=True, help_text="Detailed program information"
    )
...
class FormField(AbstractFormField):
    ...
    page = ParentalKey(
        "cms.FormPage", on_delete=models.CASCADE, related_name="form_fields"
    )
