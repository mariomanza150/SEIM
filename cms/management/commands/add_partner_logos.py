...
from cms.models import ConvenioIndexPage, ConvenioPage
from cms.utils.logo_generator import PARTNER_INSTITUTIONS, create_placeholder_logo
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logo_collection, created = Collection.objects.get_or_create(
                name="Partner Logos", defaults={"depth": root_collection.depth + 1}
            )
            if created:
                root_collection.add_child(instance=logo_collection)
                self.stdout.write(
                    self.style.SUCCESS("✓ Created 'Partner Logos' collection")
                )
        ...
        try:
            ...
            if not convenio_index:
                ...
                self.stdout.write(
                    "  Run: docker-compose exec web python manage.py populate_internacional_content"
                )
                return
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"✗ Error finding ConvenioIndexPage: {e}")
            )
            return
            ...
            try:
                ...
                if existing_image:
                    ...
                    existing_image.file.save(
                        f"{slug}.png", ImageFile(logo_buffer), save=True
                    )
                    ...
                    logo_image = Image(title=info["name"], collection=logo_collection)
                    ...
                    # Add tags
                    logo_image.tags.add(
                        "partner-logo", "institution", info["country"].lower()
                    )
                ...
                if existing_page:
                    ...
                    existing_page.agreement_type = info.get(
                        "agreement_type", "bilateral"
                    )
                    existing_page.save_revision().publish()
                    pages_updated += 1
                    self.stdout.write(
                        self.style.WARNING("  ↻ Updated existing Convenio page")
                    )
                else:
                ...
                self.stdout.write(
                    f"     Type: {info.get('agreement_type', 'bilateral')}"
                )
        ...
        self.stdout.write(
            "  http://localhost:8000/internacional/institucional/convenios/"
        )
        self.stdout.write("\nView logos in Wagtail admin:")
        self.stdout.write(
            "  http://localhost:8000/cms/ → Images → Partner Logos collection"
        )
        self.stdout.write("=" * 60)
