...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for user in staff_users:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"  ✓ Added {user.username} to Editors group")
                )
            except Group.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(
                        "  ⚠ Editors group not found. Use --create-editors-group"
                    )
                )

        self.stdout.write(self.style.SUCCESS("\nWagtail permissions setup complete!"))
        self.stdout.write(
            self.style.SUCCESS(f"Total staff users configured: {staff_users.count()}")
        )
    ...
    def create_editors_group(self):
        ...
        self.stdout.write(
            self.style.SUCCESS("  ✓ Configured Editors group permissions")
        )
    ...
    def create_moderators_group(self):
        ...
        publish_permissions = Permission.objects.filter(
            content_type=page_ct, codename__in=["publish_page"]
        )
        group.permissions.add(*publish_permissions)

        self.stdout.write(
            self.style.SUCCESS("  ✓ Configured Moderators group permissions")
        )
