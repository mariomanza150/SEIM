...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if root_page is None:
            raise RuntimeError(
                "Wagtail root page not found (Page.get_first_root_node() returned None)."
            )
        ...
        for page in default_welcome_pages:
            if page.specific_class != HomePageModel:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "  ⚠ Renamed default Wagtail welcome page (will delete later)"
                    )
                )
        ...
        for page in renamed_welcome_pages:
            try:
                page.delete()
                self.stdout.write(
                    self.style.WARNING("  ⚠ Deleted default Wagtail welcome page")
                )
            except Exception:
                # Non-fatal: welcome page cleanup should not block CMS restore.
                self.stdout.write(
                    self.style.WARNING(
                        "  ⚠ Could not delete default Wagtail welcome page; continuing"
                    )
                )
        ...
        if not BlogIndexPage.objects.filter(slug="blog").exists():
            blog_index = BlogIndexPage(
                title="Blog", slug="blog", show_in_menus=True, locale=locale
            )
        ...
        if not StandardPage.objects.filter(slug="about").exists():
            about_page = StandardPage(
                title="About SEIM", slug="about", show_in_menus=True, locale=locale
            )
        ...
        if not StandardPage.objects.filter(slug="contact").exists():
            contact_page = StandardPage(
                title="Contact Us", slug="contact", show_in_menus=True, locale=locale
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Site root: {home_page.title} (/{home_page.slug}/)")
        )
        self.stdout.write(
            self.style.SUCCESS(f"Site URL: http://{site.hostname}:{site.port}/")
        )
        self.stdout.write(
            self.style.SUCCESS(
