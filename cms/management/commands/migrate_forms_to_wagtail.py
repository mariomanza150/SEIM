...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR("No default site found. Please set up Wagtail first.")
            )
            return
        ...
        self.stdout.write(
            f"Found {form_submissions.count()} FormSubmissions to migrate"
        )
            ...
            for form_type in form_types:
                try:
                    ...
                    if existing:
                        self.stdout.write(
                            self.style.WARNING(
                                f"  FormPage already exists: {form_type.name}"
                            )
                        )
                        continue
                    ...
                    form_page = FormPage(
                        ...
                        to_address=form_type.created_by.email
                        if form_type.created_by
                        else "admin@seim.local",
                    ...
                    if form_type.schema and "properties" in form_type.schema:
                        ...
                        for field_name, field_config in properties.items():
                            ...
                            wagtail_field_type = self._map_field_type(
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"  ✓ Created FormPage: {form_page.title}")
                    )
                    migrated_forms += 1
        ...
        if dry_run:
            ...
            self.stdout.write(
                "2. Update Program.application_form references to use new FormPages"
            )
            self.stdout.write("3. Test form submissions")
            self.stdout.write(
                "4. Once verified, run: python manage.py remove_old_form_system"
            )
