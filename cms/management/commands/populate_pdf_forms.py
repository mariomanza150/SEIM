    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            forms_collection, created = Collection.objects.get_or_create(
                name="Mobility Forms", defaults={"depth": root_collection.depth + 1}
            )
            if created:
                root_collection.add_child(instance=forms_collection)
                self.stdout.write(
                    self.style.SUCCESS("✓ Created 'Mobility Forms' collection")
                )
        except:
            forms_collection = None
            self.stdout.write(
                self.style.WARNING("⚠ Could not create collection, using default")
            )
        ...
        for _form_key, form_info in FORMS.items():
            ...
            try:
                ...
                existing_doc = Document.objects.filter(title=form_info["title"]).first()

                if existing_doc:
                    # Update existing document
                    existing_doc.file.delete(save=False)
                    existing_doc.file.save(
                        form_info["filename"], ContentFile(pdf_buffer.read()), save=True
                    )
                    updated_count += 1
                    self.stdout.write(
                        self.style.WARNING("  ↻ Updated existing document")
                    )
                else:
                    # Create new document
                    doc = Document(
                        title=form_info["title"], collection=forms_collection
                    )
                    ...
                    created_count += 1
                    self.stdout.write(self.style.SUCCESS("  ✓ Created new document"))
