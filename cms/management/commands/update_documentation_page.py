    ...
    def handle(self, *args, **options):
        ...
        try:
            doc_page = StandardPage.objects.filter(
                slug="documentacion", title__icontains="Documentación"
            ).first()
        ...
        forms = Document.objects.filter(tags__name="mobility-form").order_by("title")

        if not forms.exists():
            self.stdout.write(
                self.style.ERROR("✗ No forms found! Run: populate_pdf_forms")
            )
            return
        ...
        categories = {
            "general": {"title": "Formularios Generales", "forms": []},
            "academic": {"title": "Formularios Académicos", "forms": []},
            "administrative": {"title": "Formularios Administrativos", "forms": []},
        }
        ...
        for _category_key, category_data in categories.items():
            if category_data["forms"]:
                # Category heading
                new_body.append(
                    ("rich_text", {"content": f"<h3>{category_data['title']}</h3>"})
                )
                ...
                for form in category_data["forms"]:
                    ...
                    if "Solicitud" in form.title:
                        ...
                        description = (
                            "Documento requerido para el programa de movilidad"
                        )
