...
describe('normalizeSpaLocation', () => {
  ...
  it('maps leftover root and SPA alias paths used in notifications', () => {
    ...
    expect(normalizeSpaLocation('/seim/admin/catalogs')).toEqual({
      name: 'AdminCatalogs',
    })
    ...
    expect(normalizeSpaLocation('/sessions')).toEqual({ name: 'Settings' })
    expect(normalizeSpaLocation('/seim/coordinator-dashboard')).toEqual({ name: 'Dashboard' })
